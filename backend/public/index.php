...
function dispatchRoute($method, $uri)
{
    ...
    // API: GET /api/auth/me
    if ($method === 'GET' && strpos($uri, '/api/auth/me') === 0) {
        $controller = new \App\Controllers\AuthController();
        return call_user_func([$controller, 'me']);
    }
    ...
    // API: GET /api/employees/{id}/layout
    if ($method === 'GET' && preg_match('/^\/api\/employees\/(\d+)\/layout$/', $uri, $matches)) {
        $controller = new \App\Controllers\EmployeeController();
        return call_user_func([$controller, 'getEmployeeLayout'], $matches[1]);
    }
    ...
    if (strpos($uri, '/api/attendance') === 0) {
        ...
        if ($method === 'GET') {
            ...
            if (strpos($uri, '/api/attendance/grid-save') === 0) {
                \App\Middleware\RoleMiddleware::requirePermission('Attendance', 'edit');
                $requestData = json_decode(file_get_contents('php://input'), true);
    ...
    if (strpos($uri, '/api/appraisal-templates') === 0) {
        $controller = new \App\Controllers\AppraisalTemplateController();
        if ($method === 'GET') {
            $parts = explode('/', trim($uri, '/'));
            $id = end($parts);
            if (is_numeric($id)) {
                return call_user_func([$controller, 'show'], $id);
            }
            return call_user_func([$controller, 'index']);
        } elseif ($method === 'POST') {
            $requestData = json_decode(file_get_contents('php://input'), true);
            return call_user_func([$controller, 'store'], $requestData);
        } elseif ($method === 'PUT') {
            $parts = explode('/', trim($uri, '/'));
            $id = end($parts);
            $requestData = json_decode(file_get_contents('php://input'), true);
            return call_user_func([$controller, 'update'], $id, $requestData);
        } elseif ($method === 'DELETE') {
            $parts = explode('/', trim($uri, '/'));
            $id = end($parts);
    ...
    // API: APPRAISAL MATRICES
    if (strpos($uri, '/api/appraisal-matrices') === 0) {
        \App\Middleware\RoleMiddleware::requirePermission('Appraisals', 'approve');
        $controller = new \App\Controllers\AppraisalController();
        if ($method === 'GET') {
            return call_user_func([$controller, 'getMatrices']);
        } elseif ($method === 'POST') {
            return call_user_func([$controller, 'saveMatrices']);
        }
    }

    // API: APPRAISAL CYCLES
    if (strpos($uri, '/api/appraisal-cycles') === 0) {
        $controller = new \App\Controllers\AppraisalCycleController();
        if ($method === 'GET') {
            return call_user_func([$controller, 'index']);
        } elseif ($method === 'POST') {
            $parts = explode('/', trim($uri, '/'));
            ...
            if ($action === 'generate') {
                $requestData = json_decode(file_get_contents('php://input'), true);
                return call_user_func([$controller, 'generate'], $requestData);
            } elseif ($action === 'cancel') {
                return call_user_func([$controller, 'cancel'], $parts[count($parts) - 2]);
            }
        } elseif ($method === 'PUT') {
            $parts = explode('/', trim($uri, '/'));
            $id = end($parts);
    ...
    if (strpos($uri, '/api/appraisals') === 0) {
        ...
        if ($method === 'GET') {
            ...
            $action = end($parts);
            if (is_numeric($action)) {
                return call_user_func([$controller, 'destroy'], $action);
            }
        }
    }
