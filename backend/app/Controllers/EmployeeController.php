...
class EmployeeController extends Controller
{
    ...
    public function updateEmployee($requestData)
    {
        ...
        $isAdmin = $this->hasGlobalOrRegionalScope();
        if (!$isAdmin) {
        ...
        try {
            ...
            if ($this->isGlobalAdmin()) {
                ...
                if ($viewMode === 'employee') {
                    ...
                    if ($userId) {
                        $stmtRoles = $db->prepare("SELECT COALESCE(br.name, r.name) as name FROM user_roles ur JOIN roles r ON ur.role_id = r.id LEFT JOIN roles br ON r.base_role_id = br.id WHERE ur.user_id = ?");
                        $stmtRoles->execute([$userId]);
                        $userRoles = array_map('trim', $stmtRoles->fetchAll(\PDO::FETCH_COLUMN));
                        if (in_array('Office HRAssistant', $userRoles) || in_array('Office HR Assistant', $userRoles)) {
                            $isOfficeAssistant = true;
                        }
                    }

                    if ($isOfficeAssistant) {
                        // Strict enforcement: Office HR Assistants can ONLY view employees in their own primary company
                        $sessionCompanyId = $_SESSION['scope_company_id'] ?? null;
                        
                        // Fallback: Query primary company dynamically if session is missing it
                        ...
                        // Regional Admins / Country Managers
                        $associatedCompanyIds = $_SESSION['associated_company_ids'] ?? [];
                        $sessionCountryId = $_SESSION['scope_country_id'] ?? null;
