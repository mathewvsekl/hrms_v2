...
class EmployeeService
{
    ...
    public function getEmployeeDetail(int $id, ?int $requestorEmployeeId = null, bool $isAdmin = false): array
    {
        ...
        $compStmt = $this->db->prepare("
            SELECT c.id, c.name, c.country_id, cn.name as country_name, cn.iso_code, cn.currency_code, ec.is_primary, ec.include_in_payroll, ec.is_active, ec.deactivated_at_utc 
            FROM companies c
        ...
        $stmtSalary = $this->db->prepare("SELECT * FROM salary_structures WHERE employee_id = ? ORDER BY effective_date DESC");
        $stmtSalary->execute([$id]);
        $employee['salary_structures'] = $stmtSalary->fetchAll(\PDO::FETCH_ASSOC);
    ...
    public function updateEmployee(int $employeeId, array $data, bool $isAdmin = false): bool
    {
        try {
            ...
            $allowedFields = [
                'first_name', 'last_name', 'email', 'personal_email', 'phone', 'personal_phone', 'date_of_birth', 'gender', 
            ...
            if (isset($data['company_ids'])) {
                $this->syncCompanies($employeeId, $data['company_ids'], $data['primary_company_id'] ?? null, $data['payroll_company_ids'] ?? null);
            }
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO employees (
                    ...
                    email, personal_email, phone, personal_phone, date_of_birth, gender, nationality, 
            ...
            $stmt->execute([
                ...
                $data['last_name'] ?? '',
                $data['email'] ?? '',
                (!empty($data['personal_email']) ? $data['personal_email'] : null),
                (!empty($data['phone']) ? $data['phone'] : null),
                (!empty($data['personal_phone']) ? $data['personal_phone'] : null),
            ...
            // 3. Office Links
            $payrollCompanyIds = $data['payroll_company_ids'] ?? null;
            foreach ($data['company_ids'] as $index => $compId) {
                $isPrimary = ($index === 0) ? 1 : 0;
                $includeInPayroll = 0;
                if ($payrollCompanyIds !== null) {
                    $includeInPayroll = in_array($compId, $payrollCompanyIds) ? 1 : 0;
                } else {
                    $includeInPayroll = $isPrimary;
                }
                $this->db->prepare("INSERT INTO employee_companies (employee_id, company_id, is_primary, include_in_payroll, is_active) VALUES (?, ?, ?, ?, 1)")
                         ->execute([$newId, $compId, $isPrimary, $includeInPayroll]);
            }
            ...
            if (!empty($data['send_welcome_email'])) {
                ...
                if (!empty($data['email'])) {
                    \App\Helpers\MailHelper::sendWelcomeEmail($data['email'], $userName);
                }
            }
    ...
    private function syncCompanies(int $employeeId, array $companyIds, ?int $primaryId = null, ?array $payrollCompanyIds = null): void
    {
        ...
        foreach ($existing as $compId => $isActive) {
            if (!in_array($compId, $companyIds) && $isActive) {
                $this->db->prepare("UPDATE employee_companies SET is_active = 0, is_primary = 0, include_in_payroll = 0, deactivated_at_utc = CURRENT_TIMESTAMP WHERE employee_id = ? AND company_id = ?")
        ...
        foreach ($companyIds as $index => $compId) {
            ...
            if (array_key_exists($compId, $existing)) {
                $this->db->prepare("UPDATE employee_companies SET is_active = 1, is_primary = ?, include_in_payroll = ?, deactivated_at_utc = NULL WHERE employee_id = ? AND company_id = ?")
                         ->execute([$isPrimary, $includeInPayroll, $employeeId, $compId]);
            } else {
                $this->db->prepare("INSERT INTO employee_companies (employee_id, company_id, is_primary, include_in_payroll, is_active) VALUES (?, ?, ?, ?, 1)")
                         ->execute([$employeeId, $compId, $isPrimary, $includeInPayroll]);
            }
        }
    }
     ...
     */
    public function sendWelcomeEmail(int $employeeId): void
    {
        $stmt = $this->db->prepare("SELECT e.email, e.first_name, e.last_name, u.id as user_id FROM employees e LEFT JOIN users u ON e.id = u.employee_id WHERE e.id = ?");
        $stmt->execute([$employeeId]);
        $employee = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$employee || empty($employee['email'])) {
            throw new \Exception("Employee not found or has no email address.");
        }
        ...
        $success = \App\Helpers\MailHelper::sendWelcomeEmail($employee['email'], $userName);
        ...
        if (!empty($employee['user_id'])) {
            $this->auditService->log('UPDATE', 'users', $employee['user_id'], null, ['action' => 'Welcome email sent']);
        }
    }
}
