...
class Controller
{
    ...
    protected function verifyDataScope($targetOfficeId = null, $targetCountryId = null, $targetEmployeeId = null)
    {
        ...
        try {
            ...
            if ($viewMode === 'admin') {
                $hasAdminRole = count(array_intersect(['SuperAdmin', 'Admin', 'HR Manager', 'CountryManager', 'HRAssistant', 'Office HRAssistant', 'Office HR Assistant'], $roles)) > 0;
                if (!$hasAdminRole) {
                    $viewMode = 'employee';
                }
            }
            ...
            if (in_array('HR Manager', $roles) || in_array('CountryManager', $roles) || in_array('HRAssistant', $roles) || in_array('Office HRAssistant', $roles) || in_array('Office HR Assistant', $roles)) {
    ...
    {
        $this->loadRoles();
        return $this->hasGlobalOrRegionalScope() || in_array('HRAssistant', self::$userRoles) || in_array('Office HRAssistant', self::$userRoles) || in_array('Office HR Assistant', self::$userRoles);
    }
    ...
    private function loadRoles()
    {
        ...
        if (self::$userRoles === null) {
            ...
            ");
            $stmt->execute([$userId]);
            self::$userRoles = array_map('trim', $stmt->fetchAll(\PDO::FETCH_COLUMN));
        }
    }
