...
class Controller
{
    ...
    public function jsonResponse($data = null, $httpStatus = 200, $message = "")
    {
        ...
        if ($httpStatus === 500) {
            file_put_contents(__DIR__ . '/../../500_errors.log', date('Y-m-d H:i:s') . " - [500 Error] $message\n", FILE_APPEND);
        }
    ...
    protected static $userRoleIds = null;
    ...
    protected function verifyDataScope($targetOfficeId = null, $targetCountryId = null, $targetEmployeeId = null)
    {
        ...
        try {
            $this->loadRoleIds();
            $roleIds = self::$userRoleIds;

            $isSuperAdmin = in_array(RoleConstants::SUPER_ADMIN, $roleIds, true);
            $isAdmin = in_array(RoleConstants::ADMIN, $roleIds, true);
            $isGlobalAdmin = RoleConstants::anyInGroup($roleIds, RoleConstants::MANAGEMENT);
            ...
            if ($targetEmployeeId !== null && $_SERVER['REQUEST_METHOD'] !== 'GET') {
                ...
                $hierStmt = $db->prepare("
                    ...
                    WHERE u.employee_id = ? AND COALESCE(r.base_role_id, r.id) = ?
                ");
                $hierStmt->execute([$targetEmployeeId, RoleConstants::SUPER_ADMIN]);
                if ($hierStmt->fetch()) {
            ...
            if ($viewMode === 'admin') {
                $hasAdminRole = $this->hasEntityScope();
                if (!$hasAdminRole) {
                    $viewMode = 'employee';
                }
            }
            ...
            if (RoleConstants::anyInGroup($roleIds, [RoleConstants::HR_MANAGER, RoleConstants::COUNTRY_MANAGER, RoleConstants::HR_ASSISTANT])) {
            ...
            if (in_array(RoleConstants::EMPLOYEE, $roleIds, true) && $_SERVER['REQUEST_METHOD'] !== 'GET') {
    ...
    {
        $this->loadRoleIds();
        return in_array(RoleConstants::SUPER_ADMIN, self::$userRoleIds, true);
    }
    ...
    {
        $this->loadRoleIds();
        return RoleConstants::anyInGroup(self::$userRoleIds, RoleConstants::MANAGEMENT);
    }
    ...
    {
        $this->loadRoleIds();
        return $this->isGlobalAdmin() || in_array(RoleConstants::COUNTRY_MANAGER, self::$userRoleIds, true);
    }

    /**
     * Check if user has at least Entity (Company/Office) Admin scope
     */
    protected function hasEntityScope($module = null)
    {
        $this->loadRoleIds();
        
        // Use hardcoded check as fallback for legacy logic
        if (RoleConstants::anyInGroup(self::$userRoleIds, RoleConstants::ALL_ADMIN)) {
            return true;
        }

        // Granular RBAC Check: If a specific module is requested, check if the user has view access to it.
        // This ensures the configuration as per RBAC is applied to each section or module, not entirely on/off.
        if ($module) {
            return \App\Middleware\RoleMiddleware::hasPermission($module, 'view') || \App\Middleware\RoleMiddleware::hasPermission($module, 'edit');
        }
     ...
     */
    /**
     * Load canonical role IDs for the current session user.
     * Uses RoleConstants::resolveUserRoleIds() which handles base_role_id inheritance.
     */
    private function loadRoleIds()
    {
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
        if (self::$userRoleIds === null) {
            ...
            self::$userRoleIds = RoleConstants::resolveUserRoleIds($userId);
        }
    }
