...
function dispatchRoute($method, $uri)
{
    ...
    // API: APPRAISALS CONFIG
    if (strpos($uri, '/api/appraisals/config') === 0) {
        $controller = new \App\Controllers\AppraisalController();
        if ($method === 'GET') {
            return call_user_func([$controller, 'getKPIConfigs'], $_GET);
        } elseif ($method === 'POST') {
            $requestData = json_decode(file_get_contents('php://input'), true);
            return call_user_func([$controller, 'saveKPIConfigs'], $requestData);
        }
    }
