...
-- Last Updated: 2026-03-24 (v1.6.5 AUDITED_ALIGNMENT)
...
CREATE TABLE `attendance_logs` (
  ...
  `source` ENUM('web', 'mobile', 'biometric', 'manual', 'leave_module') DEFAULT 'web',
  `status` VARCHAR(50) DEFAULT 'present' COMMENT 'Master anchor for daily reporting',
  `approval_status` ENUM('draft', 'submitted', 'approved', 'rejected') DEFAULT 'approved',
...
INSERT INTO appraisal_templates (name, description) VALUES ('Standard Annual Appraisal', 'Default annual performance review template including KPIs and 10 mandatory soft skills.');
SET @TemplateId = LAST_INSERT_ID();

INSERT INTO template_questions (template_id, section, question_text, display_order) VALUES 
