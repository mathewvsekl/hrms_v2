...
class TaxSlabsController extends Controller
{
    ...
    public function addSlab($data)
    {
        try {
            $stmt = $this->db->prepare("
                INSERT INTO tax_slabs (component_id, min_amount, max_amount, tax_type, percentage, fixed_amount, company_id)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
    ...
    public function updateSlab($id, $data)
    {
        try {
            $stmt = $this->db->prepare("
                UPDATE tax_slabs 
                SET component_id = ?, min_amount = ?, max_amount = ?, tax_type = ?, percentage = ?, fixed_amount = ?, company_id = ?
                WHERE id = ?
            ");
            $stmt->execute([
                !empty($data['component_id']) ? $data['component_id'] : null,
    ...
    public function bulkSave($data)
    {
        try {
            ...
            // Insert new slabs
            $stmt = $this->db->prepare("
                INSERT INTO tax_slabs (component_id, effective_date, min_amount, max_amount, tax_type, percentage, fixed_amount, company_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
