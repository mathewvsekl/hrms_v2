...
replacement = """                // Fetch advances
                $advStmt = $this->db->prepare("SELECT id, amount, currency_code FROM salary_advances WHERE employee_id = ? AND status = 'Pending'");
                $advStmt->execute([$emp['id']]);
                $advances = $advStmt->fetchAll(PDO::FETCH_ASSOC);
...
with open(filepath, 'w') as f:
    f.write(content)
print("Updated PayrollService.php to apply currency conversion for salary advances.")
