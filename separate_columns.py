...
target_th = """                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Gross</th>
                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Net Pay</th>"""
replacement_th = """                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Gross ({currentCurrency})</th>
                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Gross (Reporting)</th>
                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Net Pay ({currentCurrency})</th>
                                        <th style={{ padding: '16px 24px', fontSize: '13px', fontWeight: '600', color: '#64748b' }}>Total Net Pay (Reporting)</th>"""

content = content.replace(target_th, replacement_th)

# Replace the table rows
target_td = """                                        <td style={{ padding: '16px 24px', fontSize: '14px', color: '#475569' }}>
...
replacement_td = """                                        <td style={{ padding: '16px 24px', fontSize: '14px', color: '#475569' }}>
                                            <div>{currentCurrency} {formatCurrency(run.total_gross_pay)}</div>
                                        </td>
...
with open(filepath, 'w') as f:
    f.write(content)
print("Updated Payroll.jsx to separate reporting currency into its own columns.")
