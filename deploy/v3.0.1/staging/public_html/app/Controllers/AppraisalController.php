...
class AppraisalController extends Controller
{
    ...
    public function approveAppraisal($id, $data)
    {
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can approve appraisals.");
        }
        ...
        try {
            ...
            $isSuperAdmin = $this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN']);
            if ($isSuperAdmin) {
                $stmt = $this->db->prepare("UPDATE appraisal_approvals SET status = 'approved', comments = ? WHERE appraisal_id = ?");
                ...
                if ($stmt->rowCount() === 0) {
                    $this->db->rollBack();
                    return $this->jsonResponse(null, 403, "You are not authorized to approve this appraisal.");
                }
            }
    ...
    {
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can return appraisals.");
        }
