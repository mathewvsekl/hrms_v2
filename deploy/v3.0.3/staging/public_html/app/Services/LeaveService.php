        ...
        try {
            ...
            foreach ($segments as $idx => $seg) {
                ...
                if ($draftId && $idx === 0) {
                    ...
                    $newStatus = ($isSuperAdmin && $oldReq['status'] === 'approved') ? 'approved' : 'pending';
                    ...
                    if ($oldReq['status'] === 'approved' && $newStatus === 'approved') {
                        ...
                        $db->prepare("UPDATE leave_balances SET used_days = used_days - ? WHERE employee_id = ? AND leave_type_id = ? AND year = ?")
                           ->execute([$oldReq['total_days'], $employeeId, $oldReq['leave_type_id'], date('Y', strtotime($oldReq['start_date']))]);
                        // Add new days
                        $db->prepare("UPDATE leave_balances SET used_days = used_days + ? WHERE employee_id = ? AND leave_type_id = ? AND year = ?")
                           ->execute([$totalDays, $employeeId, $ltid, date('Y', strtotime($start))]);
                    }
