...
class LeaveController extends Controller
{
    ...
    private function calculateLeaveDays($companyId, $leaveTypeId, $startDate, $endDate)
    {
        ...
        $period = new \DatePeriod($start, new \DateInterval('P1D'), $end);
            ...
            foreach ($period as $dt) {
                ...
                // Skip if it's a holiday (unless sandwiched under calendar days policy)
                if (in_array($dateStr, $holidays)) {
                    if ($isCalendarDays) {
                        // Sandwich Rule: Count as leave ONLY if preceded and followed by leave days in this request
                        $before = (new \DateTime($dateStr))->modify("-1 day")->format("Y-m-d");
                        $after = (new \DateTime($dateStr))->modify("+1 day")->format("Y-m-d");

                        if ($before >= $startDate && $after <= $endDate) {
                            // Sandwiched! Count as leave (do not continue)
                        } else {
                            continue; // Not sandwiched, skip holiday
                        }
                    } else {
                        continue; // Working days policy, skip holiday
                    }
                }
    ...
    public function approveRequest($requestData)
    {
        ...
        try {
            ...
            $syncSuccess = $attendance->setLeaveAttendanceRange(
                $request['employee_id'], 
                $request['start_date'], 
                $request['end_date'],
                $leaveTypeCode
            ...
            if (!$syncSuccess) {
                // We don't necessarily want to fail the whole approval if sync fails (can be redone manually),
                // but we should at least log it or handle it. For now, we proceed as the balance was deducted.
            }
    ...
    public function getBalances($employeeId)
    {
        try {
            ...
            // Find company ID for the employee to get relevant color codes
            $stmtComp = $db->prepare("SELECT company_id FROM employee_companies WHERE employee_id = ? AND is_primary = 1 LIMIT 1");
            $stmtComp->execute([$employeeId]);
            ...
            $stmt = $db->prepare("
                ...
                LEFT JOIN office_attendance_status_definitions sd ON lt.code = sd.status_key AND sd.company_id = :cid
                WHERE lb.employee_id = :eid AND lb.year = :year
            ");
            $stmt->execute(['eid' => $employeeId, 'year' => $year, 'cid' => $companyId]);
    ...
    public function addLeaveType($data)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("INSERT INTO leave_types (name, code, is_paid, gender_restriction) VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $data['name'], 
                $data['code'], 
                $data['is_paid'] ? 1 : 0,
    ...
    public function updateLeaveType($data)
    {
        ...
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("UPDATE leave_types SET name = ?, code = ?, is_paid = ?, gender_restriction = ? WHERE id = ?");
            $stmt->execute([
                $data['name'], 
                $data['code'], 
                $data['is_paid'] ? 1 : 0,
    ...
    private function recalculateBalances($companyId, $employeeId = null)
    {
        ...
        $stmt = $db->prepare("SELECT leave_type_id, default_days_per_year FROM company_leave_policies WHERE company_id = ?");
        $stmt->execute([$companyId]);
        ...
        foreach ($employees as $eid) {
            foreach ($policies as $policy) {
                ...
                // Get leave type code to match in attendance logs
                $stmtLT = $db->prepare("SELECT code FROM leave_types WHERE id = ?");
                $stmtLT->execute([$ltid]);
                $ltCode = $stmtLT->fetchColumn();

                // Calculate used days from attendance_logs for this year
                $stmtUsed = $db->prepare("
                    SELECT COUNT(*) 
                ...
                ");
                $stmtUsed->execute([$eid, $ltCode, $year]);
                $used = $stmtUsed->fetchColumn();
