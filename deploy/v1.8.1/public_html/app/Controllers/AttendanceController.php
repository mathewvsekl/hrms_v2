...
class AttendanceController extends Controller
{
     ...
     */
    public function setLeaveAttendanceRange($employeeId, $startDate, $endDate, $status = 'on_leave')
    {
        try {
            $db = \Database::getInstance()->getConnection();
    ...
    public function getAttendanceStatuses($requestData)
    {
        try {
            ...
            // 1. Fetch Leave Types with their company-specific colors if defined
            $stmt = $db->prepare("
                SELECT lt.code as id, lt.name, lt.gender_restriction, 'leave' as type, sd.color_code
                FROM leave_types lt
                LEFT JOIN office_attendance_status_definitions sd ON lt.code = sd.status_key AND sd.company_id = ?
            ...
            $systemDefaults = [
                ...
                'work_from_home' => ['id' => 'work_from_home', 'name' => 'Work From Home', 'type' => 'system', 'color_code' => '#6366f1'],
                'on_site' => ['id' => 'on_site', 'name' => 'On Site', 'type' => 'system', 'color_code' => '#06b6d4'],
                'training' => ['id' => 'training', 'name' => 'Training', 'type' => 'system', 'color_code' => '#8b5cf6'],
                'weekend' => ['id' => 'weekend', 'name' => 'Weekend', 'type' => 'system', 'color_code' => '#6b7280'],
                'public_holiday' => ['id' => 'public_holiday', 'name' => 'Public Holiday', 'type' => 'system', 'color_code' => '#ec4899']
            ];
    ...
    public function getGridLogs($requestData)
    {
        try {
            ...
            $query = "
                ...
                LEFT JOIN employee_companies ec ON e.id = ec.employee_id AND ec.is_primary = 1 AND ec.is_active = 1
                LEFT JOIN companies c ON ec.company_id = c.id
                LEFT JOIN public_holidays ph ON c.country_id = ph.country_id AND ph.holiday_date = :date
                LEFT JOIN office_attendance_status_definitions def ON c.id = def.company_id AND def.is_default = 1
