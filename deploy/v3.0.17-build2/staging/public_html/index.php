...
if ($requestMethod === 'GET' && $requestUri === '/api/run-migration') {
    $db = \Database::getInstance()->getConnection();
    try {
        $db->exec("ALTER TABLE employee_companies ADD COLUMN include_in_payroll TINYINT(1) DEFAULT 0 AFTER is_primary");
        $db->exec("UPDATE employee_companies SET include_in_payroll = 1 WHERE is_primary = 1");
    } catch (Exception $e) {}

    try {
        $db->exec("ALTER TABLE payroll_records ADD COLUMN reporting_currency VARCHAR(10) NULL AFTER net_pay");
    } catch (Exception $e) {}
    try {
        $db->exec("ALTER TABLE payroll_records ADD COLUMN exchange_rate DECIMAL(10,2) NULL AFTER reporting_currency");
    } catch (Exception $e) {}
    try {
        $db->exec("ALTER TABLE payroll_records ADD COLUMN company_id INT NOT NULL DEFAULT 1 AFTER employee_id");
    } catch (Exception $e) {}
    try {
    ...
    try {
        $db->exec("ALTER TABLE employee_salary_components ADD COLUMN currency_code VARCHAR(3) DEFAULT 'UGX' AFTER effective_date");
    } catch (Exception $e) {}
    try {
        $db->exec("ALTER TABLE salary_structures ADD COLUMN currency_code VARCHAR(3) DEFAULT 'UGX' AFTER other_earnings");
    } catch (Exception $e) {}
    try {
    ...
    try {
        $db->exec("ALTER TABLE company_leave_policies ADD COLUMN `year` INT(4) NOT NULL DEFAULT 2026 AFTER `company_id`");
        echo json_encode(['status' => 'success', 'message' => 'Migrations executed successfully']);
    } catch (Exception $e) {
        if (strpos($e->getMessage(), 'Duplicate column name') !== false) {
            echo json_encode(['status' => 'success', 'message' => 'Migrations already executed']);
        } else {
            http_response_code(500);
