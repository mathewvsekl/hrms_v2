...
class LeaveService
{
    ...
    public function submitRequest(array $data, ?string $attachmentPath): array
    {
        ...
        try {
            ...
            foreach ($segments as $idx => $seg) {
                ...
                if (!$draftId && $totalDays > $remaining) {
                ...
                if ($draftId && $idx === 0) {
                    $stmtUpd = $db->prepare("
                        ...
                        SET leave_type_id = :ltid, start_date = :start, end_date = :end, total_days = :days, status = 'pending', remarks = :remarks, attachment_path = COALESCE(:attachment, attachment_path)
                        WHERE id = :draft_id AND employee_id = :eid AND status = 'draft'
                    ");
                    $stmtUpd->execute([
                        'ltid' => $ltid,
                        'start' => $start,
    ...
    public function generateSystemDraftLeaves(?int $companyId = null, ?int $employeeId = null): int
    {
        ...
        $stmtLt = $db->query("
            SELECT clp.company_id, lt.id, lt.code 
            FROM leave_types lt
    ...
    private function isWeekendOrHoliday(int $companyId, string $date): bool
    {
        ...
        $stmt = $db->prepare("
            ...
            LEFT JOIN holidays ch ON c.id = ch.company_id AND ch.holiday_date = ?
            WHERE c.id = ?
        ");
        $stmt->execute([$dayOfWeek, $date, $date, $companyId]);
    ...
    private function saveDraftLeaveGroup(array $group): void
    {
        ...
        $stmtIns = $db->prepare("
            INSERT INTO leave_requests (employee_id, leave_type_id, start_date, end_date, total_days, status, request_group_id, remarks, origin)
            VALUES (:eid, :ltid, :start, :end, :days, 'draft', :gid, 'System-Generated Leave Request', 'system')
        ");
        $stmtIns->execute([
