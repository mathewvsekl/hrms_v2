...
class AttendanceService
{
     ...
     */
    public function saveManualEntry(array $data, int $actionByUserId): array
    {
        $db = \Database::getInstance()->getConnection();
        ...
        if ($config['attendance_mode'] === 'time_based' && $checkInUtc && $status === 'present') {
            $checkInTime = new \DateTime($checkInUtc);
     ...
     */
    public function saveBulkEntry(array $data, int $actionByUserId): array
    {
        $db = \Database::getInstance()->getConnection();
    ...
    public function autoPersistDefaults(string $date): int
    {
        ...
        $stmt = $db->prepare($query);
        $stmt->execute([
            'date_lr' => $date, 'date_ph' => $date, 'date_ch' => $date,
    ...
    public function getMonthlyReport(array $params): array
    {
        ...
        $systemCodes = [
            ...
            'holiday' => ['key' => 'HO', 'label' => 'Holiday', 'color_hex' => '#2563eb'],
            'present' => ['key' => 'PR', 'label' => 'Present', 'color_hex' => '#10b981'],
            'absent' => ['key' => 'AB', 'label' => 'Absent', 'color_hex' => '#ef4444'],
        ];

        $statusQuery = "SELECT status_key, status_label, color_code, display_code FROM office_attendance_status_definitions";
        $statusParams = [];
        if ($companyId) {
        ...
        foreach ($stmtL->fetchAll(\PDO::FETCH_ASSOC) as $l) {
            $systemCodes[$l['code']] = ['key' => $l['code'], 'label' => $l['name'], 'color_hex' => $l['color_code'] ?? '#6b7280'];
        }
        ...
        // DEBUG: Trace query and params
        error_log("AttendanceService DEBUG: Query: " . preg_replace('/\s+/', ' ', $empQuery));
        error_log("AttendanceService DEBUG: Params: " . json_encode($empParams));
        ...
        $chMap = [];
        $stmtCh = $db->prepare("SELECT company_id, holiday_date FROM holidays WHERE holiday_date BETWEEN ? AND ?");
        $stmtCh->execute([$startDate, $endDate]);
        foreach ($stmtCh->fetchAll(\PDO::FETCH_ASSOC) as $ch) $chMap[$ch['company_id']][] = $ch['holiday_date'];
        ...
        foreach ($employees as $emp) {
            ...
            foreach ($datesMeta as $dm) {
                ...
                if (!$status) {
                    ...
                    elseif (isset($chMap[$cid]) && in_array($d, $chMap[$cid])) $status = 'holiday';
