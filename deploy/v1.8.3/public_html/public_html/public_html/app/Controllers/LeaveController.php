...
class LeaveController extends Controller
{
    ...
    private function recalculateBalances($companyId, $employeeId = null)
    {
        ...
        $stmt = $db->prepare("
            SELECT clp.leave_type_id, clp.default_days_per_year, lt.name as leave_type_name 
        ...
        foreach ($employees as $eid) {
            foreach ($policies as $policy) {
                ...
                // We handle both slug codes (AL) and potentially full names to be robust, 
                // but standardizing on lt.code
                $stmtUsed = $db->prepare("
                    SELECT COUNT(*) 
                ...
                ");
                $stmtUsed->execute([$eid, $ltCode, $policy['leave_type_name'] ?? $ltCode, $year]);
