...
class LeaveController extends Controller
{
    ...
    public function addHoliday($requestData)
    {
        ...
        try {
            ...
            $stmt = $db->prepare("
                ...
                SET al.status = 'PH_SYS', al.source = 'holiday_sync'
            ...
            $stmt = $db->prepare("
                ...
                SELECT e.id, :date, 'PH_SYS', 'approved', 'holiday_sync'
                FROM employees e
                JOIN employee_companies ec ON e.id = ec.employee_id AND ec.is_active = 1
    ...
    public function getLeaveTypes($employeeId = null)
    {
        try {
            ...
            // Add company filtering if passed or relying on session
            $companyId = $_GET['company_id'] ?? ($_SESSION['scope_company_id'] ?? null);
            $all = $_GET['all'] ?? 0;
            
            if (!$all && $companyId) {
                // If not requesting all (for template copy), limit to company or global
    ...
    public function addLeaveType($data)
    {
        try {
            ...
            $companyId = $data['company_id'] ?? ($_SESSION['scope_company_id'] ?? null);

            $name = $data['name'] ?? 'Leave';
            $isPaid = $data['is_paid'] ?? 1;
            $genderRestriction = $data['gender_restriction'] ?? 'none';
            $colorCode = $data['color_code'] ?? '#6b7280';

            // Copy logic
            if (!empty($data['copy_from_id'])) {
                $stmtOrig = $db->prepare("SELECT * FROM leave_types WHERE id = ?");
                $stmtOrig->execute([$data['copy_from_id']]);
                $orig = $stmtOrig->fetch(\PDO::FETCH_ASSOC);
                if ($orig) {
                    $name = $orig['name'];
                    $isPaid = $orig['is_paid'];
                    $genderRestriction = $orig['gender_restriction'];
                    $colorCode = $orig['color_code'];
                }
            }

            // Generate logical display code
            $words = explode(' ', $name);
            $displayCode = '';
            foreach ($words as $w) {
            ...
            if (strlen($displayCode) < 2) {
                $displayCode = strtoupper(substr(str_replace(' ', '', $name), 0, 2));
            }
            ...
            $finalCode = $displayCode . '_' . $isoCode;

            // Ensure unique final code
            $baseCode = $finalCode;
            $counter = 1;
            while(true) {
                $chk = $db->prepare("SELECT id FROM leave_types WHERE code = ?");
                $chk->execute([$finalCode]);
                if (!$chk->fetchColumn()) break;
                $finalCode = $baseCode . $counter;
                $counter++;
            }

            $stmt = $db->prepare("INSERT INTO leave_types (name, code, display_code, company_id, is_paid, gender_restriction, color_code) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $name, 
                $finalCode, 
                $displayCode,
            ...
            return $this->jsonResponse(['id' => $db->lastInsertId(), 'code' => $finalCode, 'display_code' => $displayCode], 201, "Leave type added.");
        } catch (\Exception $e) {
            return $this->jsonResponse(null, 500, "Error: " . $e->getMessage());
        }
    }
