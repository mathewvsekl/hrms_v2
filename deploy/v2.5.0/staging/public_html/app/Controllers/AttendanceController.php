...
class AttendanceController extends Controller
{
    ...
    public function saveManualEntry($requestData)
    {
        ...
        // Fetch ISO code for the country
        $iso = 'SYS';
        $db = \Database::getInstance()->getConnection();
        $stmtIso = $db->prepare("
            SELECT countries.iso_code 
            FROM countries 
            JOIN companies ON countries.id = companies.country_id 
            JOIN employee_companies ON companies.id = employee_companies.company_id 
            WHERE employee_companies.employee_id = :eid AND employee_companies.is_primary = 1
            LIMIT 1
        ");
        $stmtIso->execute(['eid' => $employeeId]);
        $iso = $stmtIso->fetchColumn() ?: 'SYS';

        $status = $requestData['status'] ?? ('PR_' . $iso);
        
        // Map legacy status strings to dynamic ISO keys
        $legacyMap = [
            'present' => 'PR_',
            'absent' => 'AB_',
            'late' => 'LT_',
        ...
        if (isset($legacyMap[strtolower($status)])) {
            $status = $legacyMap[strtolower($status)] . $iso;
        } elseif (strpos($status, '_SYS') !== false) {
            $status = str_replace('_SYS', '_' . $iso, $status);
        }
        ...
        try {
            ...
            // Logic: Process status based on mode if not explicitly set (handle legacy 'PR_SYS' or missing prefix)
            if ($config && $config['attendance_mode'] === 'time_based' && !empty($checkIn) && (strpos($status, 'PR_') === 0)) {
    ...
    public function saveBulkEntry($requestData)
    {
        ...
        $status = $requestData['status'] ?? 'PR_SYS';
        $checkIn = !empty($requestData['check_in']) ? $requestData['check_in'] : null;
        $checkOut = !empty($requestData['check_out']) ? $requestData['check_out'] : null;
        ...
        try {
            ...
            foreach ($employeeIds as $employeeId) {
                // Fetch ISO code for this employee
                $stmtIso = $db->prepare("
                    SELECT countries.iso_code 
                    FROM countries 
                    JOIN companies ON countries.id = companies.country_id 
                ...
                if (isset($legacyMap[strtolower($finalStatus)])) {
                    $finalStatus = $legacyMap[strtolower($finalStatus)] . $iso;
                } elseif (strpos($finalStatus, '_SYS') !== false) {
                    $finalStatus = str_replace('_SYS', '_' . $iso, $finalStatus);
                }
                ...
                if ($oldValues) {
                    ...
                    $stmt = $db->prepare("
                        INSERT INTO attendance_logs (employee_id, attendance_date, status, check_in_utc, check_out_utc, remarks, approval_status, source, is_manually_modified, actor_type)
                        VALUES (:eid, :date, :status, :check_in_utc, :check_out_utc, :remarks, 'approved', 'manual', 1, 'user')
                    ");
                    $stmt->execute([
                        'eid' => $employeeId,
                        'date' => $date,
    ...
    public function setLeaveAttendanceRange($employeeId, $startDate, $endDate, $status = 'on_leave', $isWorkingDayOnly = false, $companyId = null)
    {
        try {
            ...
            if ($isWorkingDayOnly && $companyId) {
                // Fetch weekends
                $stmt = $db->prepare("SELECT day_of_week FROM office_weekly_schedules WHERE company_id = ? AND status = 'WE_SYS'");
        ...
        try {
            ...
            if ($emp) {
                ...
                foreach ($pubHolidays as $ph) {
                    $holidays[$ph['holiday_date']] = ['status' => 'PH_SYS', 'name' => $ph['name']];
                }
                ...
                foreach ($compHolidays as $ch) {
                    $holidays[$ch['holiday_date']] = ['status' => 'HO_SYS', 'name' => $ch['name']];
                }
            ...
            $iso = 'SYS';
            if ($emp) {
                $stmtIso = $db->prepare("SELECT iso_code FROM countries WHERE id = ?");
                $stmtIso->execute([$emp['country_id']]);
                $iso = $stmtIso->fetchColumn() ?: 'SYS';
            }
                ...
                if (isset($logsByDate[$dateStr])) {
                    ...
                    if (isset($leavePolicies[$st])) {
                        ...
                        if (!$isCalendarDays && ($isWeekend || $isHoliday)) {
                            ...
                            $log['status'] = $isHoliday ? ($holidays[$dateStr]['status'] === 'PH_SYS' ? 'PH_'.$iso : 'HO_'.$iso) : 'WE_'.$iso;
                    ...
                    $enrichedLogs[] = [
                        ...
                        'status' => $holidays[$dateStr]['status'] === 'PH_SYS' ? 'PH_'.$iso : 'HO_'.$iso,
    ...
    public function getAttendanceStatuses($requestData)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            $companyId = $requestData['company_id'] ?? null;
            $fetchAll = $requestData['all'] ?? 0;
            
            // RBAC: Verify user has access to this company context
            if ($companyId) $this->verifyDataScope($companyId);
            
            // 1. Fetch Leave Types
            $stmt = $db->prepare("
                SELECT lt.code as id, lt.name, lt.gender_restriction, 'leave' as type, lt.color_code, lt.display_code
                FROM leave_types lt
                WHERE lt.company_id = ? OR lt.company_id IS NULL
            ...
            $isoCode = 'SYS';
            if ($companyId) {
                $stmtIso = $db->prepare("SELECT c.iso_code FROM companies comp JOIN countries c ON comp.country_id = c.id WHERE comp.id = ?");
                $stmtIso->execute([$companyId]);
                $foundIso = $stmtIso->fetchColumn();
                if ($foundIso) $isoCode = $foundIso;
            }

            // 2. Define System Defaults
            $systemDefaults = [
                'PR_' . $isoCode => ['id' => 'PR_' . $isoCode, 'name' => 'Present', 'type' => 'system', 'display_code' => 'PR', 'color_code' => '#10b981'],
                'AB_' . $isoCode => ['id' => 'AB_' . $isoCode, 'name' => 'Absent', 'type' => 'system', 'display_code' => 'AB', 'color_code' => '#ef4444'],
                'WH_' . $isoCode => ['id' => 'WH_' . $isoCode, 'name' => 'Work From Home', 'type' => 'system', 'display_code' => 'WH', 'color_code' => '#d97706'],
                'OS_' . $isoCode => ['id' => 'OS_' . $isoCode, 'name' => 'On Site', 'type' => 'system', 'display_code' => 'OS', 'color_code' => '#059669'],
                'TR_' . $isoCode => ['id' => 'TR_' . $isoCode, 'name' => 'Training', 'type' => 'system', 'display_code' => 'TR', 'color_code' => '#8b5cf6'],
                'WE_' . $isoCode => ['id' => 'WE_' . $isoCode, 'name' => 'Weekend', 'type' => 'system', 'display_code' => 'WE', 'color_code' => '#9ca3af'],
                'PH_' . $isoCode => ['id' => 'PH_' . $isoCode, 'name' => 'Public Holiday', 'type' => 'system', 'display_code' => 'PH', 'color_code' => '#3b82f6'],
                'HO_' . $isoCode => ['id' => 'HO_' . $isoCode, 'name' => 'Holiday', 'type' => 'system', 'display_code' => 'HO', 'color_code' => '#3b82f6']
            ];

            $officeStatuses = [];
            if ($companyId) {
                $stmt = $db->prepare("SELECT id, status_key, status_label as name, display_code, 'attendance' as type, color_code, is_default 
            ...
            $allDefinitions = array_values($systemDefaults);
            if ($fetchAll) {
                $stmt = $db->prepare("SELECT MIN(id) as id, status_label as name, MIN(display_code) as display_code, MIN(color_code) as color_code FROM office_attendance_status_definitions GROUP BY status_label");
                $stmt->execute();
                $allDefinitions = array_merge($allDefinitions, $stmt->fetchAll(\PDO::FETCH_ASSOC));
            }
            ...
            return $this->jsonResponse([
                ...
                'leave' => $leaveTypes,
                'all' => $all,
                'system_templates' => $allDefinitions
            ]);
        } catch (\Exception $e) {
    ...
    public function saveAttendanceStatusDefinition($requestData)
    {
        try {
            ...
            // Copy Logic Overrides
            $isNew = false;
            if (!empty($requestData['copy_from_id'])) {
                $stmtOrig = $db->prepare("SELECT * FROM office_attendance_status_definitions WHERE id = ?");
                $stmtOrig->execute([$requestData['copy_from_id']]);
                $orig = $stmtOrig->fetch(\PDO::FETCH_ASSOC);
                if ($orig) {
                    $statusLabel = $orig['status_label'];
                    $color = $orig['color_code'];
                    $isNew = true;
                }
            } else {
                // Determine if new based on status_key existence
                if ($statusKey) {
                    $chk = $db->prepare("SELECT id FROM office_attendance_status_definitions WHERE company_id = ? AND status_key = ?");
                    $chk->execute([$companyId, $statusKey]);
                    if (!$chk->fetchColumn()) $isNew = true;
                } else {
                    $isNew = true;
                }
            }

            if (!$companyId || !$statusLabel) {
                return $this->jsonResponse(null, 400, "Company ID and Label are required.");
            }
            ...
            $words = explode(' ', $statusLabel);
            $displayCode = '';
            foreach ($words as $w) {
                if (!empty($w)) $displayCode .= strtoupper(substr($w, 0, 1));
            }
            if (strlen($displayCode) < 2) {
                $displayCode = strtoupper(substr(str_replace(' ', '', $statusLabel), 0, 2));
            }
            $displayCode = substr($displayCode, 0, 2);

            // If it's new, format status_key as [DISPLAY]_[ISO]
            if ($isNew) {
                $isoCode = 'SYS';
                $stmtIso = $db->prepare("SELECT c.iso_code FROM companies comp JOIN countries c ON comp.country_id = c.id WHERE comp.id = ?");
                $stmtIso->execute([$companyId]);
                ...
                $finalKey = $displayCode . '_' . $isoCode;
                $baseKey = $finalKey;
                $counter = 1;
                while(true) {
                    $chk = $db->prepare("SELECT id FROM office_attendance_status_definitions WHERE company_id = ? AND status_key = ?");
                    $chk->execute([$companyId, $finalKey]);
            ...
            $stmt = $db->prepare("
                INSERT INTO office_attendance_status_definitions (company_id, status_key, display_code, status_label, color_code, is_default, sort_order)
                VALUES (:cid, :key, :dcode, :label, :color, :is_def, :sort)
                ON DUPLICATE KEY UPDATE 
                    display_code = :dcode_up,
            ...
            $db->commit();
            return $this->jsonResponse(['message' => "Status definition saved successfully.", 'status_key' => $statusKey, 'display_code' => $displayCode]);
        } catch (\Exception $e) {
            if ($db->inTransaction()) $db->rollBack();
            ...
            $query = "
                SELECT 
                    ...
                    COALESCE(
                        al.status, 
                        lr.leave_code,
                        IF(ph.id IS NOT NULL, CONCAT('PH_', cnt.iso_code), NULL),
                        IF(ch.id IS NOT NULL, CONCAT('HO_', cnt.iso_code), NULL),
                        IF(ws.status IN ('Off', 'Weekend', 'Holiday'), CONCAT('WE_', cnt.iso_code), 
                           IF(ws.id IS NULL AND :day_name_weekend IN ('Saturday', 'Sunday'), CONCAT('WE_', cnt.iso_code), CONCAT('PR_', cnt.iso_code))
                        )
                    ) as status,
                    al.approval_status,
                    al.remarks,
    ...
    public function saveGridEntries($requestData)
    {
        ...
        try {
            ...
            foreach ($entries as $entry) {
                ...
                // Fetch ISO code for dynamic default
                $stmtIso = $db->prepare("
                    SELECT countries.iso_code 
                    FROM countries 
                    JOIN companies ON countries.id = companies.country_id 
    ...
    public function autoPersistDefaults($requestData = [])
    {
        try {
            ...
            $isoMap = [];
            $stmtIso = $db->query("SELECT comp.id, c.iso_code FROM companies comp JOIN countries c ON comp.country_id = c.id");
            while ($row = $stmtIso->fetch(\PDO::FETCH_ASSOC)) {
                $isoMap[$row['id']] = $row['iso_code'];
            }

            foreach ($toProcess as $row) {
                $iso = $isoMap[$row['company_id']] ?? 'SYS';
     ...
     */
    private function getMonthlyReportData($requestData)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            ...
            // Build Universal Map for this request context
            $systemCodes = [];
            
            // Default mapping for "Display" purposes in reports
            $defaultDisplay = [
                'PR_' => ['key' => 'PR', 'label' => 'Present', 'color' => 'bg-emerald-100 text-emerald-800'],
                'AB_' => ['key' => 'AB', 'label' => 'Absent', 'color' => 'bg-rose-100 text-rose-800'],
                'WH_' => ['key' => 'WH', 'label' => 'Work From Home', 'color' => 'bg-indigo-100 text-indigo-800'],
                'OS_' => ['key' => 'OS', 'label' => 'On Site', 'color' => 'bg-cyan-100 text-cyan-800'],
                'WE_' => ['key' => 'WE', 'label' => 'Weekend', 'color' => 'bg-orange-100 text-orange-800'],
                'PH_' => ['key' => 'PH', 'label' => 'Holiday', 'color' => 'bg-sky-100 text-sky-800'],
                'HO_' => ['key' => 'HO', 'label' => 'Holiday', 'color' => 'bg-sky-100 text-sky-800'],
                'TR_' => ['key' => 'TR', 'label' => 'Training', 'color' => 'bg-violet-100 text-violet-800'],
            ];

            // Fetch ISO codes for all companies to build systemCodes
            $isoMap = [];
            $stmtIso = $db->query("SELECT comp.id, c.iso_code FROM companies comp JOIN countries c ON comp.country_id = c.id");
            while ($row = $stmtIso->fetch(\PDO::FETCH_ASSOC)) {
                $isoMap[$row['id']] = $row['iso_code'];
            ...
            // Build systemCodes for every company found in the report
            foreach ($isoMap as $cid => $iso) {
                foreach ($defaultDisplay as $prefix => $data) {
                    $fullKey = $prefix . $iso;
                    $systemCodes[$fullKey] = $data;
                }
            }

            // Fetch Office Attendance Status
            $officeStatusQuery = "SELECT company_id, status_key, status_label, color_code, display_code FROM office_attendance_status_definitions";
            $officeStatusParams = [];
            if ($companyId) {
            ...
            foreach ($officeStatuses as $s) {
                ...
                $systemCodes[$statusKey] = [
                    'key' => $s['display_code'] ?: strtoupper($statusKey),
            ...
            // Fetch Leave Types
            // Temporarily removed 'color' assuming it may be missing from db schema locally.
            $leaveTypeQuery = "SELECT code, name FROM leave_types";
            ...
            $empQuery = "
                SELECT e.id as employee_id, e.employee_code, e.first_name, e.last_name, 
                       c.country_id, c.id as company_id
                FROM employees e
                JOIN employee_companies ec ON e.id = ec.employee_id AND ec.is_primary = 1 AND ec.is_active = 1
            ...
            // Restructure Logs by Employee -> Date
            $logsMap = [];
            foreach ($logs as $log) {
            ...
            $leaveQuery = "
                ...
                JOIN employee_companies ec ON e.id = ec.employee_id AND ec.is_primary = 1 AND ec.is_active = 1
                WHERE lr.status = 'approved' AND (
                    lr.start_date <= ? AND lr.end_date >= ?
                )
            ";
            $leaveParams = [$endDate, $startDate];
            if ($companyId) {
                $leaveQuery .= " AND ec.company_id = ?";
                $leaveParams[] = $companyId;
            }
            ...
            foreach ($employees as $emp) {
                ...
                $employeeRow = [
                    ...
                    'days' => [],
                    'totals' => [] // calculated on frontend or here
                ];

                // Check weekends for this company
                $companyWeekends = $weekendMap[$cid] ?? ['Saturday', 'Sunday']; // defaults
                $iso = $isoMap[$cid] ?? 'SYS';
                ...
                for ($d = 1; $d <= $daysInMonth; $d++) {
                    ...
                    // priority: Leave > Log > Public Holiday > Company Holiday > Weekend
    ...
    /**
     * Endpoint API for returning JSON of the Monthly Report
     */
    public function getMonthlyReport($requestData)
    {
        try {
            $data = $this->getMonthlyReportData($requestData);
            return $this->jsonResponse($data);
        } catch (\Exception $e) {
            return $this->jsonResponse(null, 500, "Error rendering report: " . $e->getMessage());
        }
    }
    ...
    {
        try {
            // Re-use logic to get the grid
            // But since this is a GET request, we need to extract from $_GET
            $requestData = $_GET; // Override
            
            // Re-use logic to get the grid
            $dataData = $this->getMonthlyReportData($requestData);
            
            // To match existing variable structure downstream:
            $data = ['data' => $dataData];
            
            if (!$data || !isset($data['data']['grid'])) {
            ...
            $daysInMonth = $data['data']['days_in_month'];
            ...
            // Headers
            $headers = ['Employee Name', 'Employee ID'];
            for ($i = 1; $i <= $daysInMonth; $i++) {
                $dateStr = "$year-$month-" . str_pad($i, 2, '0', STR_PAD_LEFT);
                $dayName = substr(date('l', strtotime($dateStr)), 0, 3);
                $headers[] = "$i ($dayName)";
            }
            ...
            foreach ($gridData as $row) {
                ...
                for ($i = 1; $i <= $daysInMonth; $i++) {
                    $statusRaw = $row['days'][$i];
                    $displayCode = '';
                    
                    if (strpos($statusRaw, 'WE_') === 0) {
                        $displayCode = '-';
                    } else if ($statusRaw) {
                         // Attempt to find friendly code mapping, default to raw code
                         $displayCode = $data['data']['config'][$statusRaw]['key'] ?? strtoupper(substr($statusRaw, 0, 3));
                    }
