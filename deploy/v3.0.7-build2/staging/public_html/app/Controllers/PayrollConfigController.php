...
class PayrollConfigController extends Controller
{
    ...
    public function addComponent($data)
    {
        try {
            ...
            $stmt = $this->db->prepare("
                INSERT INTO payroll_components (name, type, computation_type, value, formula, company_id, country_id, is_statutory, is_non_taxable, status, display_in_payslip)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['name'],
                ...
                !empty($data['is_non_taxable']) && $data['type'] === 'EARNING' ? 1 : 0,
                $data['status'] ?? 'Active',
    ...
    public function updateComponent($id, $data)
    {
        try {
            ...
            $stmt = $this->db->prepare("
                ...
                SET name = ?, type = ?, computation_type = ?, value = ?, formula = ?, company_id = ?, country_id = ?, is_non_taxable = ?, status = ?, display_in_payslip = ?
                WHERE id = ?
            ");
            $stmt->execute([
                $data['name'],
        ...
        try {
            ...
            $stmt = $this->db->prepare("
                SELECT esc.*, pc.name as component_name, pc.type as component_type, 
                       pc.computation_type, pc.is_statutory
                FROM employee_salary_components esc
                JOIN payroll_components pc ON esc.component_id = pc.id
                WHERE esc.employee_id = ?
                ORDER BY esc.effective_date DESC, pc.type ASC, pc.name ASC
            ");
    ...
    public function saveEmployeeComponents($data)
    {
        try {
            ...
            if (!$employeeId || !$effectiveDate || empty($components)) {
                return $this->jsonResponse(null, 400, 'employee_id, effective_date, and components are required');
            }

            $this->db->beginTransaction();

            // Delete existing entries for this employee + effective_date
            $stmtDel = $this->db->prepare("DELETE FROM employee_salary_components WHERE employee_id = ? AND effective_date = ?");
            $stmtDel->execute([$employeeId, $effectiveDate]);
    ...
    public function deleteEmployeeComponentsByDate()
    {
        try {
            ...
            if (!$employeeId || !$effectiveDate) {
                return $this->jsonResponse(null, 400, 'employee_id and effective_date are required');
            }

            $stmt = $this->db->prepare("DELETE FROM employee_salary_components WHERE employee_id = ? AND effective_date = ?");
