...
class AppraisalController extends Controller
{
    ...
    public function getAppraisal($id)
    {
        ...
        try {
            ...
            // Fetch history
            $history = ApprovalHelper::getHistory('appraisal', $id);
    ...
    public function initiateCycle($data)
    {
        ...
        try {
            ...
            foreach ($employees as $emp) {
                ...
                // Log history
                ApprovalHelper::log('appraisal', $appraisalId, $user['id'], 'initiated', 'Appraisal cycle initiated.');
            }
    ...
    public function submitToManager($id)
    {
        ...
        // Log history
        ApprovalHelper::log('appraisal', $id, $user['id'], 'submitted', 'Appraisal submitted to manager for review.');
    ...
    public function approveAppraisal($id, $data)
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("UPDATE appraisal_approvals SET status = 'approved', comment = ? WHERE appraisal_id = ? AND approver_id = ?");
            $stmt->execute([$comment, $id, $managerId]);

            // Log history
            ApprovalHelper::log('appraisal', $id, $user['id'], 'approved', $comment);
    ...
    public function returnAppraisal($id, $data)
    {
        ...
        try {
            ...
            $this->db->prepare("UPDATE appraisal_approvals SET status = 'returned', comment = ? WHERE appraisal_id = ? AND approver_id = ?")->execute([$comment, $id, $managerId]);

            // Log history
            ApprovalHelper::log('appraisal', $id, $user['id'], 'returned', $comment);
    ...
    public function finalize($id, $data)
    {
        ...
        try {
            ...
            // Log history
            $user = \App\Middleware\AuthMiddleware::getUser();
            ApprovalHelper::log('appraisal', $id, $user['id'], 'finalized', "Finalized with rating: $rating");
