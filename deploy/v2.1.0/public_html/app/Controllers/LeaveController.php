...
class LeaveController extends Controller
{
    ...
    public function submitRequest($requestData)
    {
        ...
        try {
            ...
            foreach ($segments as $seg) {
                ...
                if ($actorId) {
                    ApprovalHelper::log('leave', $lastId, $actorId, 'submitted', "Leave request submitted for {$totalDays} days.");
                }
            }
    ...
    public function approveRequest($requestData)
    {
        ...
        try {
            ...
            // Log history
            ApprovalHelper::log('leave', $id, $approverId, 'approved', 'Leave request approved.');
    ...
    public function rejectRequest($requestData)
    {
        ...
        try {
            ...
            // Log history
            ApprovalHelper::log('leave', $id, $approverId, 'rejected', $comment);
    ...
    public function adminCancel($requestData)
    {
        ...
        try {
            ...
            // Log history
            $actorId = $_SESSION['user_id'] ?? null;
            ApprovalHelper::log('leave', $id, $actorId, 'cancelled', $comment);
    ...
    /**
     * Get history for a specific leave request
     */
    public function getRequestHistory($requestData)
    {
        $id = $requestData['id'] ?? null;
        if (!$id) return $this->jsonResponse(null, 400, "Request ID required.");

        $history = ApprovalHelper::getHistory('leave', $id);
