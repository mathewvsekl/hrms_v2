...
class AttendanceController extends Controller
{
    ...
    public function submitLogs($requestData)
    {
        ...
        try {
            ...
            // Log individual history for each ID
            foreach ($ids as $id) {
                ApprovalHelper::log('attendance', $id, $userId, 'submitted', 'Manual log submitted for approval.');
            }
    ...
    public function reviewLogs($requestData)
    {
        ...
        try {
            ...
            // Log individual history for each ID
            foreach ($ids as $id) {
                ApprovalHelper::log('attendance', $id, $userId, $action, $remarks);
            }
    ...
    public function getAuditHistory($requestData)
    {
        ...
        try {
            ...
            // 2. Fetch from Approval History
            $approvalHistory = ApprovalHelper::getHistory('attendance', $logId);

            // 3. Merge and Sort
            $fullHistory = array_merge($auditHistory, $approvalHistory);
            usort($fullHistory, function($a, $b) {
                return strtotime($b['created_at_utc']) - strtotime($a['created_at_utc']);
            });

            return $this->jsonResponse($fullHistory);
        } catch (\Exception $e) {
            return $this->jsonResponse(null, 500, "Database error: " . $e->getMessage());
        }
    }
