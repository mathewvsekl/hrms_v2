...
class ExportController extends Controller
{
    ...
    public function exportData()
    {
        ...
        if (!$this->isGlobalAdmin() && !\App\Middleware\RoleMiddleware::hasPermission('Reports', 'view')) {
    ...
    private function fetchAttendanceCsv($db, $companyId, $start, $end)
    {
        $stmt = $db->prepare("
            SELECT al.attendance_date, e.employee_code, e.first_name, e.last_name, 
                   al.status, al.check_in_utc, al.check_out_utc, al.source, al.remarks, e.id as employee_id
        ...
        foreach ($rows as &$row) {
            ...
            $row['check_in_local'] = \App\Helpers\DateHelper::toLocal($row['check_in_utc'] ?? null, $tz, 'Y-m-d H:i:s');
            $row['check_out_local'] = \App\Helpers\DateHelper::toLocal($row['check_out_utc'] ?? null, $tz, 'Y-m-d H:i:s');
            
            unset($row['check_in_utc']);
            unset($row['check_out_utc']);
    ...
    private function fetchPayrollCsv($db, $companyId, $start, $end)
    {
        ...
        $stmt = $db->prepare("
            SELECT pr.year, pr.month, e.employee_code, e.first_name, e.last_name,
                   pr.net_pay, pr.gross_chargeable_income, pr.status as run_status
