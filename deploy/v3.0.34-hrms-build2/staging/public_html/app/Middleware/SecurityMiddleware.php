    ...
    public static function interceptRestrictedPaths()
    {
        ...
        foreach ($restrictedEndpoints as $endpoint) {
            if (strpos($uri, $endpoint) === 0) {
                ...
                // These are needed for standard employee features (e.g. leave application categories, company settings)
                $method = $_SERVER['REQUEST_METHOD'] ?? 'GET';
                $isInformational = ($method === 'GET' && in_array($endpoint, ['/api/leave/types', '/api/holidays', '/api/leave/policies', '/api/organization/settings']));
                ...
                // Role ID-based check using centralized constants as fallback
                $isAdministrativeRole = \App\Helpers\RoleConstants::isInGroup($roleId, \App\Helpers\RoleConstants::ALL_ADMIN);
                
                // Overhaul: Check RBAC permissions dynamically PER MODULE. 
                // Only grant administrative bypass if the user has permissions for the specific section being accessed.
                if (!$isAdministrativeRole) {
                    $endpointToModuleMap = [
                        '/api/leave/recalculate' => 'leave',
                        '/api/leave/types' => 'leave',
                        '/api/leave/policies' => 'leave',
                        '/api/holidays' => 'holidays',
                        '/api/appraisals/settings' => 'appraisals',
                        '/api/rbac' => 'configuration',
                        '/api/organization/settings' => 'configuration'
                    ];

                    foreach ($endpointToModuleMap as $prefix => $module) {
                        if (strpos($uri, $prefix) === 0) {
                            if (\App\Middleware\RoleMiddleware::hasPermission($module, 'view') || \App\Middleware\RoleMiddleware::hasPermission($module, 'edit')) {
                ...
                // Allow administrative roles to bypass restricted context (Employee View) 
                // for these specific system endpoints to prevent getting "stuck".
                if ($isAdministrativeRole && !$isImpersonating) {
                    continue; 
                }
