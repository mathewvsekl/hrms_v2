...
class MailHelper
{
    ...
    public static function send(string $email, string $subject, string $message): bool
    {
        ...
        // 3. Safety check: If PHPMailer class is missing (e.g. vendor folder issue), avoid a Fatal Error
        if (!class_exists('PHPMailer\PHPMailer\PHPMailer')) {
            if (ENVIRONMENT === 'development') {
                error_log("MailHelper: PHPMailer library missing. OTP logged to file instead.");
                return true; 
            }
            throw new \Exception("Email delivery system is currently unavailable (Missing Library).");
        }
