...
class AuditPDO extends PDO
{
    ...
    public static function flushLogs(AuditPDO $pdo)
    {
        ...
        if (empty(self::$auditLogs)) {
            @file_put_contents($debugLog, date('Y-m-d H:i:s') . " - flushLogs called but self::\$auditLogs is empty.\n", FILE_APPEND);
            return;
        }

        @file_put_contents($debugLog, date('Y-m-d H:i:s') . " - flushLogs starting with " . count(self::$auditLogs) . " entries.\n", FILE_APPEND);
        ...
        try {
            ...
            @file_put_contents($debugLog, date('Y-m-d H:i:s') . " - Successfully flushed logs.\n", FILE_APPEND);
        } catch (Exception $e) {
            @file_put_contents($debugLog, date('Y-m-d H:i:s') . " - EXCEPTION in flushLogs: " . $e->getMessage() . "\n", FILE_APPEND);
