...
class UploadHelper
{
    ...
    public static function upload($file, $context = 'documents', $options = [])
    {
        ...
        if ($context === 'avatars') {
            ...
            $allowedExtensions = $allowedExtensions ?: ['pdf', 'jpg', 'jpeg', 'png', 'docx'];
            $uploadSubDir = '/public/uploads/leave/';
        } elseif ($context === 'reference_docs') {
            $allowedExtensions = ['pdf', 'jpg', 'jpeg', 'png', 'docx', 'xlsx', 'txt', 'csv', 'pptx'];
            $uploadSubDir = '/public/uploads/reference_docs/';
        } else {
        ...
        if (empty($extension) || !in_array($extension, $allowedExtensions)) {
            return ['success' => false, 'message' => 'Security Alert: File type [.' . ($extension ?: 'unknown') . '] is strictly prohibited.'];
        }

        // 3. Validation: MIME type (Robust check)
        $mime = null;
        ...
        if ($mime) {
            ...
            foreach ($forbiddenMimes as $forbidden) {
                if (stripos($mime, $forbidden) !== false) {
        ...
        // 4. Prepare Directory
        $uploadSubPath = str_replace('/public', '', $uploadSubDir);
        $uploadDir = ROOT_PATH . '/private/public' . $uploadSubPath;
        
        if (!is_dir($uploadDir)) {
            if (!mkdir($uploadDir, 0755, true)) {
                return ['success' => false, 'message' => 'Failed to create private upload directory.'];
            }
        }

        // 5. Generate Unique Filename (Sanitized and Obfuscated)
    ...
    public static function delete($dbPath)
    {
        ...
        $absPathPrivate = ROOT_PATH . '/private/public' . $cleanPath;
        
        if (file_exists($absPathPrivate) && is_file($absPathPrivate)) {
            return unlink($absPathPrivate);
        }
        return false;
    }
