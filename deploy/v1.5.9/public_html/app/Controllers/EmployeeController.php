...
class EmployeeController extends Controller
{
    ...
    {
        // Run soft migration for ENUM if necessary without throwing fatal errors
        try {
            $db = \Database::getInstance()->getConnection();
            $db->exec("ALTER TABLE employees MODIFY COLUMN status ENUM('active', 'inactive', 'onboarding', 'offboarding', 'pending_approval') DEFAULT 'active'");
        } catch (\Exception $e) {
        }
    }
    /**
        ...
        try {
            ...
            if (isset($requestData['role_id'])) {
                ...
                if ($userId) {
                    ...
                    $roleCheck = $db->prepare("SELECT 1 FROM user_roles WHERE user_id = ?");
                    $roleCheck->execute([$userId]);
                    if ($roleCheck->fetch()) {
                        $roleUpd = $db->prepare("UPDATE user_roles SET role_id = ? WHERE user_id = ?");
                        $roleUpd->execute([$roleId, $userId]);
                    } else {
    ...
    public function save($requestData)
    {
        ...
        try {
            ...
            $roleId = $requestData['role_id'] ?? 5; // Default to EMPLOYEE (ID 5) if not specified
    ...
    public function listEmployees()
    {
        try {
            ...
            if ($context !== 'onboarding') {
                $whereClause = "WHERE e.status NOT IN ('onboarding', 'pending_approval')";
            }
            ...
            $stmt = $db->prepare("
                SELECT e.id, e.first_name, e.last_name, e.employee_code, e.email, e.status, 
                       e.hire_date, e.employment_type, e.custom_data, e.phone, e.date_of_birth,
                       d.name as department_name,
