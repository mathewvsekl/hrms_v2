...
class OrganizationController extends Controller
{
    public function __construct()
    {
        try {
            ...
            $db->exec("ALTER TABLE companies MODIFY COLUMN attendance_mode ENUM('time_based', 'status_based', 'standard', 'flexible', 'strict') DEFAULT 'standard'");
        } catch (\Exception $e) {
        }
    }
    ...
    public function updateCompany($id)
    {
        ...
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("UPDATE companies SET country_id = ?, name = ?, timezone = ?, address = ?, contact_phone = ?, contact_email = ?, attendance_mode = ? WHERE id = ?");
            $stmt->execute([$data['country_id'], $data['name'], $data['timezone'], $data['address'] ?? null, $data['contact_phone'] ?? null, $data['contact_email'] ?? null, $data['attendance_mode'] ?? 'standard', $id]);
