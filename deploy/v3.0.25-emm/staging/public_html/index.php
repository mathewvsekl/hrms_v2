...
if ($normalizedUri === '/api/media') {
    // 1. Session Enforcement
    // We allow passing token via query string for <img> tags
    $token = $_GET['token'] ?? '';
    ...
    $filePath = $_GET['file'] ?? '';
    // Prevent directory traversal: remove '../' and './'
    $cleanPath = str_replace(['../', './', '..\\', '.\\'], '', $filePath);
    $cleanPath = str_replace(['/HRMS%20V2/', '/HRMS V2/'], '/', $cleanPath);
    
    // Ensure it starts with /public/uploads
    if (strpos($cleanPath, '/public/uploads') !== 0 && strpos($cleanPath, 'public/uploads') !== 0) {
        http_response_code(403);
        echo "Invalid path request";
        exit;
    }
    
    // The Nginx alias is `/private/public/uploads/`
    // So `/protected_internal/` maps directly to `/private/public/uploads/`
    $internalSuffix = str_replace('/public/uploads/', '', '/' . ltrim($cleanPath, '/'));
    $internalUri = '/protected_internal/' . ltrim($internalSuffix, '/');
    
    $absPath = ROOT_PATH . '/private/public/uploads/' . ltrim($internalSuffix, '/');
