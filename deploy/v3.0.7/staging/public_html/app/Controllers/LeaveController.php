...
class LeaveController extends Controller
{
    ...
    public function submitRequest($requestData)
    {
        ...
        try {
            ...
            $sessionData = \App\Helpers\AuthHelper::getSessionData();
            $userRole = strtoupper($sessionData['user_role'] ?? 'EMPLOYEE');
            $data['is_super_admin'] = in_array($userRole, ['SUPERADMIN', 'SUPER_ADMIN']);
    ...
    public function approveRequest($requestData)
    {
        ...
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can approve requests.");
        }
    ...
    public function rejectRequest($requestData)
    {
        ...
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can reject requests.");
        }
    ...
    {
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can cancel requests.");
        }
