...
class DashboardController extends Controller
{
    public function getSummary()
    {
        ...
        try {
            ...
            $roleId = (int)($_SESSION['role_id'] ?? \App\Helpers\RoleConstants::EMPLOYEE);
            $isMultiOfficeRole = \App\Helpers\RoleConstants::isInGroup($roleId, \App\Helpers\RoleConstants::MULTI_OFFICE);
            ...
            return $this->jsonResponse(null, 500, "Dashboard data error: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
        }
    }
    ...
    private function getPendingApprovals()
    {
        ...
        try {
            ...
            $isGlobalAdmin = in_array($roleId, [\App\Helpers\RoleConstants::SUPER_ADMIN], true);
            ...
            $appraisals = $appraisalService->listAppraisals(array_merge($user ?: [], $_SESSION), $_SESSION['user_role'] ?? 'Employee');
            $targetStatuses = [];
            if (\App\Middleware\RoleMiddleware::hasPermission('Appraisals', 'approve')) {
                $targetStatuses = ['manager_review', 'hr_review', 'l1_review'];
            } else {
                $targetStatuses = ['manager_review', 'l1_review'];
            }
