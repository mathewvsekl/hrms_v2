...
class AttendanceService
{
    ...
    public function fetchLogs(array $filters, array $sessionData, bool $isGlobalAdmin): array
    {
        ...
        if (!$isGlobalAdmin) {
            ...
            $roleId = (int)($sessionData['role_id'] ?? \App\Helpers\RoleConstants::EMPLOYEE);
            error_log("AttendanceService DEBUG: role_id=$roleId country=$sessionCountryId assoc_count=" . count($associatedCompanyIds));
            $isMultiOffice = \App\Helpers\RoleConstants::isInGroup($roleId, \App\Helpers\RoleConstants::MULTI_OFFICE);
            ...
            } else if ($roleId === \App\Helpers\RoleConstants::COUNTRY_MANAGER && $sessionCountryId) {
    ...
    public function submitLogs(array $ids, int $userId, array $scopeFilterData): int
    {
        ...
        if (!$scopeFilterData['is_global_admin']) {
            ...
            $roleId = (int)($scopeFilterData['role_id'] ?? \App\Helpers\RoleConstants::EMPLOYEE);
    ...
    public function getMonthlyReport(array $params, array $sessionData = []): array
    {
        ...
        // 2. Leave Codes
        $leaveParams = [];
        $leaveQuery = "SELECT DISTINCT lt.code, lt.name, lt.color_code FROM leave_types lt";
        
        if ($companyId) {
            $leaveQuery .= " JOIN company_leave_policies clp ON lt.id = clp.leave_type_id WHERE clp.company_id = ?";
            ...
            $leaveQuery .= " JOIN company_leave_policies clp ON lt.id = clp.leave_type_id WHERE clp.company_id IN ($placeholders)";
            $leaveParams = array_merge($leaveParams, $cIds);
        }

        $stmtL = $db->prepare($leaveQuery);
        $stmtL->execute($leaveParams);
