...
class AuthMiddleware
{
    ...
    public static function protect()
    {
        ...
        // 2. Handle Token Authentication (Bearer)
        ...
        if (!isset($_SESSION['associated_company_ids']) || !isset($_SESSION['user_role']) || !isset($_SESSION['scope_country_id'])) {
            try {
                $stmt = $db->prepare("
                ...
                if ($user && $user['is_active'] == 1) {
                    ...
                    // Role Refresh
                    $roleStmt = $db->prepare("SELECT r.name FROM roles r JOIN user_roles ur ON r.id = ur.role_id WHERE ur.user_id = ? LIMIT 1");
