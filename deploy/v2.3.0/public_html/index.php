...
// DEBUG: Force-enable error display to solve persistent 500 errors
...
if (strpos($requestUri, '/api/') !== 0 && strpos($requestUri, 'api/') !== 0) {
    // If we stripped too much, or if it's already relative
...
function dispatchRoute($method, $uri)
{
    ...
    if (strpos($uri, '/api/attendance') === 0) {
        ...
        if ($method === 'GET') {
            ...
            if (strpos($uri, '/api/attendance/status-definitions') === 0) {
                $parts = explode('/', trim($uri, '/'));
                $id = end($parts);
                return call_user_func([$controller, 'deleteAttendanceStatusDefinition'], ['id' => $id]);
            }
    ...
    if (strpos($uri, '/api/leave') === 0 || strpos($uri, '/api/holidays') === 0) {
        ...
        if (strpos($uri, '/api/leave/types') === 0) {
            if ($method === 'GET') {
                return call_user_func([$controller, 'getLeaveTypes'], $_GET['employee_id'] ?? null);
            } elseif ($method === 'POST') {
                $requestData = json_decode(file_get_contents('php://input'), true);
    ...
    if (strpos($uri, '/api/documents') === 0) {
        $controller = new \App\Controllers\DocumentController();
        if ($method === 'GET') {
            return call_user_func([$controller, 'getEmployeeDocuments'], $_GET['employee_id'] ?? null);
        } elseif ($method === 'POST') {
            return call_user_func([$controller, 'uploadDocument'], $_POST['employee_id'] ?? null);
        } elseif ($method === 'DELETE') {
            $parts = explode('/', trim($uri, '/'));
            $id = end($parts);
            return call_user_func([$controller, 'deleteDocument'], $id);
