...
class LeaveController extends Controller
{
        ...
        try {
            ...
            $query = "
                SELECT lr.*, 
                       ...
                       appr.first_name as approved_by_name
                FROM leave_requests lr
                JOIN employees e ON lr.employee_id = e.id
                JOIN leave_types lt ON lr.leave_type_id = lt.id
                LEFT JOIN employees appr ON lr.approved_by_id = appr.id
                WHERE 1=1
            ";
            ...
            if ($officeId) {
                $query .= " AND e.office_id = :office_id";
    ...
    public function submitRequest($requestData)
    {
        ...
        try {
            ...
            // 1. Get Employee Details (Office, Year, Gender)
            $stmt = $db->prepare("SELECT office_id, gender FROM employees WHERE id = :id");
            ...
            if (!$officeId)
                return $this->jsonResponse(null, 404, "Employee office not found.");
            ...
            $totalDays = $this->calculateLeaveDays($officeId, $leaveTypeId, $startDate, $endDate);
     ...
     */
    private function calculateLeaveDays($officeId, $leaveTypeId, $startDate, $endDate)
    {
        $db = \Database::getInstance()->getConnection();

        // 1. Get Office Policy (Weekend definition)
        $stmt = $db->prepare("SELECT weekend_definition, is_calendar_days FROM office_leave_policies WHERE office_id = :oid AND leave_type_id = :ltid");
        $stmt->execute(['oid' => $officeId, 'ltid' => $leaveTypeId]);
        ...
        $stmt = $db->prepare("SELECT holiday_date FROM holidays WHERE office_id = :oid AND holiday_date BETWEEN :start AND :end");
        $stmt->execute(['oid' => $officeId, 'start' => $startDate, 'end' => $endDate]);
    ...
    /**
     * Fetch public holidays for an office
     */
    public function getHolidays($officeId)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("SELECT * FROM holidays WHERE office_id = :office_id ORDER BY holiday_date ASC");
    ...
    public function addHoliday($requestData)
    {
        ...
        if (!$officeId || !$date || !$name) {
            return $this->jsonResponse(null, 400, "Office ID, Date, and Name are required.");
        }

        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("
                INSERT INTO holidays (office_id, holiday_date, name, is_recurring)
                VALUES (:office_id, :holiday_date, :name, :is_recurring)
            ");
            $stmt->execute([
                'office_id' => $officeId,
                'holiday_date' => $date,
    ...
    public function previewLeaveDays($requestData)
    {
        ...
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("SELECT office_id FROM employees WHERE id = :id");
            $stmt->execute(['id' => $employeeId]);
            ...
            $total = $this->calculateLeaveDays($officeId, $leaveTypeId, $startDate, $endDate);
    ...
    /**
     * Get leave policies for an office
     */
    public function getPolicies($officeId)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("SELECT * FROM office_leave_policies WHERE office_id = ?");
    ...
    public function savePolicy($data)
    {
        try {
            ...
            $stmt = $db->prepare("SELECT id FROM office_leave_policies WHERE office_id = ? AND leave_type_id = ?");
            $stmt->execute([$data['company_id'], $data['leave_type_id']]);
            $existingId = $stmt->fetchColumn();

            if ($existingId) {
                $stmt = $db->prepare("
                    UPDATE office_leave_policies 
                ...
                $stmt = $db->prepare("
                    INSERT INTO office_leave_policies (office_id, leave_type_id, default_days_per_year, carry_forward_allowed, is_calendar_days, weekend_definition)
                    VALUES (?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $data['company_id'],
