...
class SearchController extends Controller
{
    ...
    public function search()
    {
        ...
        try {
            ...
            $isAdmin = $this->hasAnyRole(['Admin', 'HRManager', 'HR_MANAGER', 'HR MANAGER', 'HRAssistant', 'HR_ASSISTANT', 'HR ASSISTANT']);
            ...
            if (!$isSuperAdmin) {
                ...
                $empWhere .= " AND NOT EXISTS (
                    ...
                    WHERE u2.employee_id = e.id AND (UPPER(r2.name) = 'SUPERADMIN' OR UPPER(r2.name) = 'SUPER_ADMIN')
                )";
            ...
            $allShortcuts = [
                ['name' => 'Add Employee', 'path' => '/onboarding', 'roles' => ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER', 'HR_ASSISTANT']],
                ['name' => 'Leave Requests', 'path' => '/leave', 'roles' => ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER', 'HR_ASSISTANT', 'COUNTRY_MANAGER']],
                ['name' => 'Attendance Logs', 'path' => '/attendance', 'roles' => ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER', 'HR_ASSISTANT', 'COUNTRY_MANAGER']],
                ['name' => 'Payroll Dashboard', 'path' => '/payroll', 'roles' => ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER']],
                ['name' => 'Asset Inventory', 'path' => '/assets', 'roles' => ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER', 'HR_ASSISTANT']],
                ['name' => 'System Settings', 'path' => '/admin', 'roles' => ['SUPER_ADMIN', 'ADMIN']],
                ['name' => 'My Profile', 'path' => '/employee-profile', 'roles' => ['EMPLOYEE', 'HR_MANAGER', 'ADMIN', 'SUPER_ADMIN']],
                ['name' => 'Submit Leave', 'path' => '/leave', 'roles' => ['EMPLOYEE', 'HR_MANAGER', 'ADMIN', 'SUPER_ADMIN']],
            ];

            foreach ($allShortcuts as $shortcut) {
                if (stripos($shortcut['name'], $query) !== false) {
                    if ($this->hasAnyRole($shortcut['roles'])) {
                        $results['shortcuts'][] = [
                            'name' => $shortcut['name'],
                            'path' => $shortcut['path']
                        ];
