...
class AttendanceController extends Controller
{
    ...
    public function saveManualEntry($requestData)
    {
        ...
        $isSuperAdmin = in_array(strtoupper($userRole), ['SUPERADMIN', 'SUPER_ADMIN']);
    ...
    {
        if (!$this->hasAnyRole(['SUPERADMIN', 'SUPER_ADMIN'])) {
            return $this->jsonResponse(null, 403, "Strictly restricted: Only Super Admins can review logs.");
        }
    ...
    public function saveGridEntries($requestData)
    {
        ...
        try {
            ...
            foreach ($entries as $entry) {
                ...
                $stmt = $db->prepare("SELECT id, check_in_utc, check_out_utc FROM attendance_logs WHERE employee_id = ? AND attendance_date = ?");
                ...
                $isWorkingStatus = in_array($status, ['present', 'late', 'half_day', 'on_site', 'work_from_home', 'training']);
