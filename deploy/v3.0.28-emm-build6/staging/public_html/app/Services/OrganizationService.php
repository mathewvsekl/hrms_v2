...
class OrganizationService
{
    ...
    public function listCountries(?int $sessionCountryId = null): array
    {
        if ($sessionCountryId) {
            $stmt = $this->db->prepare("SELECT * FROM countries WHERE id = ? ORDER BY name ASC");
            $stmt->execute([$sessionCountryId]);
    ...
    public function updateExchangeRate(int $id, array $data): bool
    {
        $stmt = $this->db->prepare("UPDATE exchange_rates SET from_currency = ?, to_currency = ?, rate = ?, effective_date = ?, is_active = ? WHERE id = ?");
        $success = $stmt->execute([
            $data['from_currency'], $data['to_currency'], $data['rate'], $data['effective_date'], $data['is_active'] ?? 1, $id
        ]);
        if ($success) {
            $this->auditService->log('UPDATE', 'exchange_rates', $id, null, $data);
        }
        return $success;
    }
