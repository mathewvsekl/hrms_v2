...
class PayrollService
{
    ...
    public function generatePayroll(int $month, int $year, int $companyId = 1, array $excludedEmployeeIds = [], ?string $reportingCurrency = null, ?float $exchangeRate = null): array
    {
        ...
        try {
            ...
            foreach ($employees as $emp) {
                ...
                // Fetch advances
                $advStmt = $this->db->prepare("SELECT id, amount, currency_code FROM salary_advances WHERE employee_id = ? AND status = 'Approved'");
    ...
    public function deleteRecord(int $id): array
    {
        try {
            ...
            $advStmt = $this->db->prepare("UPDATE salary_advances SET deducted_in_payroll_id = NULL, status = 'Approved' WHERE deducted_in_payroll_id = ?");
            $advStmt->execute([$id]);
