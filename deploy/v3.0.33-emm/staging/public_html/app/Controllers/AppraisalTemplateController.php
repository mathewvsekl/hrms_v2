    ...
    public function store($data)
    {
        ...
        try {
            ...
            if (!empty($data['soft_skills'])) {
                $stmtQ = $this->db->prepare("INSERT INTO template_questions (template_id, section, question_text, description, rating_scale_max, display_order) VALUES (?, 'B_SOFT_SKILL', ?, ?, ?, ?)");
                foreach ($data['soft_skills'] as $idx => $skill) {
                    $stmtQ->execute([
                        $templateId,
                        $skill['skill_name'],
                        $skill['description'] ?? null,
                        $skill['rating_scale_max'] ?? 5,
                        $idx + 1
                    ]);
                }
            }
