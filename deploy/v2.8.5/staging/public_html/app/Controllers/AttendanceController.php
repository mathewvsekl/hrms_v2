...
class AttendanceController extends Controller
{
    ...
    public function getCountries()
    {
        try {
            ...
            $query = "SELECT DISTINCT cnt.id, cnt.name, cnt.iso_code 
                      FROM countries cnt";
            $params = [];
            ...
            if (!$this->isGlobalAdmin()) {
                ...
                if (!empty($myCompanyIds)) {
                    ...
                    $query .= " JOIN companies comp ON cnt.id = comp.country_id 
    ...
    public function saveAttendanceStatusDefinition($requestData)
    {
        try {
            ...
            $stmt = $db->prepare("
                ...
                ON DUPLICATE KEY UPDATE status_label = :label, color_code = :color, is_default = :is_def, sort_order = :sort, is_deleted = 0
            ");
            $stmt->execute(['cid' => $companyId, 'key' => $statusKey, 'label' => $statusLabel, 'color' => $color, 'is_def' => $isDefault, 'sort' => $sortOrder]);
    ...
    public function saveWeeklySchedule($requestData)
    {
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO office_weekly_schedules (company_id, day_of_week, status, remarks) 
                                  VALUES (:cid, :day, :status, :remarks) ON DUPLICATE KEY UPDATE status = :status, remarks = :remarks");
            $stmt->execute(['cid' => $companyId, 'day' => $requestData['day_of_week'], 'status' => $requestData['status'], 'remarks' => $requestData['remarks'] ?? null]);
        ...
        try {
            ...
            $query = "
                SELECT 
                    ...
                    COALESCE(al.status, lr.leave_code, IF(ph.id IS NOT NULL, 'public_holiday', NULL), IF(ch.id IS NOT NULL, 'holiday', NULL),
                             IF(ws.status IN ('Off', 'Weekend', 'Holiday'), 'weekend', IF(ws.id IS NULL AND :day1 IN ('Saturday', 'Sunday'), 'weekend', 'present'))) as status,
                ...
                LEFT JOIN (SELECT lr_in.employee_id, lt_in.code as leave_code FROM leave_requests lr_in JOIN leave_types lt_in ON lr_in.leave_type_id = lt_in.id WHERE lr_in.status = 'approved' AND :date1 BETWEEN lr_in.start_date AND lr_in.end_date) lr ON e.id = lr.employee_id
                LEFT JOIN public_holidays ph ON c.country_id = ph.country_id AND ph.holiday_date = :date2
                LEFT JOIN holidays ch ON c.id = ch.company_id AND ch.holiday_date = :date3
                LEFT JOIN office_weekly_schedules ws ON c.id = ws.company_id AND ws.day_of_week = :day2
                LEFT JOIN attendance_logs al ON e.id = al.employee_id AND al.attendance_date = :date4
                WHERE e.status = 'active'
            ";

            $params = ['day1' => $dayOfWeek, 'day2' => $dayOfWeek, 'date1' => $date, 'date2' => $date, 'date3' => $date, 'date4' => $date];
    ...
    public function saveGridEntries($requestData)
    {
        ...
        try {
            ...
            foreach ($entries as $entry) {
                ...
                $stmt = $db->prepare("SELECT id FROM employee_companies WHERE employee_id = ? AND is_primary = 1 AND is_active = 1");
                $stmt->execute([$employeeId]);
                $cid = $stmt->fetchColumn();
                if (!$cid) continue;

                $stmt = $db->prepare("INSERT INTO attendance_logs (employee_id, company_id, attendance_date, status, remarks, approval_status, source, is_manually_modified, actor_type) 
                                      VALUES (:eid, :cid, :date, :st, :rem, 'approved', 'manual', 1, 'user')
                                      ON DUPLICATE KEY UPDATE status = :st2, remarks = :rem2, is_manually_modified = 1");
                $stmt->execute(['eid' => $employeeId, 'cid' => $cid, 'date' => $date, 'st' => $entry['status'], 'rem' => $entry['remarks'] ?? '', 'st2' => $entry['status'], 'rem2' => $entry['remarks'] ?? '']);
                $updatedCount++;
            }
            $db->commit();
