...
class Controller
{
    ...
    protected function verifyDataScope($targetOfficeId = null, $targetCountryId = null, $targetEmployeeId = null)
    {
        ...
        try {
            ...
            if ($viewMode === 'admin') {
                if (!in_array('SUPERADMIN', $roles) && !in_array('SUPER_ADMIN', $roles) && !in_array('ADMIN', $roles) && !in_array('HRMANAGER', $roles) && !in_array('HR_MANAGER', $roles) && !in_array('HRASSISTANT', $roles) && !in_array('HR_ASSISTANT', $roles) && !in_array('COUNTRYMANAGER', $roles) && !in_array('COUNTRY_MANAGER', $roles)) {
                    // Noah Audit Fix: Auto-fallback to employee view if user lacks admin roles
                    $viewMode = 'employee';
                }
            }

            if (in_array('HRMANAGER', $roles) || in_array('HR_MANAGER', $roles) || in_array('HRASSISTANT', $roles) || in_array('HR_ASSISTANT', $roles) || in_array('COUNTRY MANAGER', $roles) || in_array('COUNTRY_MANAGER', $roles) || in_array('COUNTRYMANAGER', $roles)) {
