...
class SalaryAdvanceController
{
    ...
    public function handleRequest($method, $path)
    {
        ...
        if ($method === 'PUT' && isset($pathParts[1]) && isset($pathParts[2]) && $pathParts[2] === 'terms') {
            return $this->updateTerms((int)$pathParts[1]);
        }

        if ($method === 'GET' && isset($pathParts[1]) && isset($pathParts[2]) && $pathParts[2] === 'installments') {
            return $this->getInstallments((int)$pathParts[1]);
        }
    ...
    private function createAdvance()
    {
        ...
        $installmentAmount = isset($input['installment_amount']) && $input['installment_amount'] !== '' ? (float)$input['installment_amount'] : null;
        $deductionStartDate = isset($input['deduction_start_date']) && $input['deduction_start_date'] !== '' ? $input['deduction_start_date'] : null;
        ...
        $stmt = $this->db->prepare("
            INSERT INTO salary_advances (employee_id, amount, installment_amount, deduction_start_date, currency_code, date_requested, reason, attachment, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Pending')
        ");
        $stmt->execute([$employeeId, $amount, $installmentAmount, $deductionStartDate, $currencyCode, $dateRequested, $reason, $attachmentPath]);
    ...
    private function updateAdvance($id)
    {
        ...
        $stmt = $this->db->prepare("UPDATE salary_advances SET amount = ?, installment_amount = ?, reason = ? WHERE id = ?");
        $stmt->execute([$amount, $installmentAmount, $reason, $id]);
    ...
    private function getInstallments($id)
    {
        $stmt = $this->db->prepare("SELECT sai.*, CONCAT(pr.month, '/', pr.year) as payroll_name, CONCAT(pr.year, '-', LPAD(pr.month, 2, '0'), '-01') as payroll_date FROM salary_advance_installments sai JOIN payroll_records pr ON sai.payroll_id = pr.id WHERE sai.salary_advance_id = ? ORDER BY sai.created_at DESC");
        $stmt->execute([$id]);
    ...
    private function updateTerms($id)
    {
        $input = json_decode(file_get_contents('php://input'), true);
        ...
        if (!$advance || !in_array(strtolower($advance['status']), ['approved', 'partially deducted'])) {
            http_response_code(400);
            return ['success' => false, 'message' => 'Terms can only be updated for active advances'];
        }

        $stmt = $this->db->prepare("UPDATE salary_advances SET installment_amount = ?, deduction_start_date = ? WHERE id = ?");
        $stmt->execute([$installmentAmount, $deductionStartDate, $id]);

        return ['success' => true, 'message' => 'Advance terms updated successfully'];
    }
}
