    ...
    public function getRequests($requestData)
    {
        try {
            ...
            if ($companyId) {
                ...
                $query .= " AND lr.employee_id IN (SELECT employee_id FROM employee_companies WHERE company_id = :company_id AND is_primary = 1 AND is_active = 1)";
                $params['company_id'] = $companyId;
            } else if (!$this->isSuperAdmin()) {
                // Noah Audit Fix: Mandatory isolation if no companyId provided
                $sessionCompanyId = $_SESSION['scope_company_id'] ?? null;
                if ($sessionCompanyId) {
                    $query .= " AND lr.employee_id IN (SELECT employee_id FROM employee_companies WHERE company_id = :session_company_id AND is_primary = 1 AND is_active = 1)";
