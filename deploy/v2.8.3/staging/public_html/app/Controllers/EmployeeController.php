...
class EmployeeController extends Controller
{
    ...
    public function getEmployee($id)
    {
        ...
        try {
            ...
            if (!$isAdmin && $employee['id'] != $myEmployeeId) {
                // Security Audit Fix: Mask sensitive identification, contact, and financial fields
    ...
    public function updateEmployee($requestData)
    {
        ...
        try {
            ...
            if (!empty($updates)) {
                ...
                // Log to Approval History if status changed
                if (isset($requestData['status'])) {
                    $action = 'updated';
                    if ($requestData['status'] === 'active') $action = 'approved';
                    if ($requestData['status'] === 'onboarding') $action = 'rejected';
                    if ($requestData['status'] === 'pending_approval') $action = 'submitted';
                    
                    ApprovalHelper::log('onboarding', $employeeId, $action, $requestData['comment'] ?? null);
                }
            }
    ...
    public function save($requestData)
    {
        ...
        try {
            ...
            $empStmt->execute([
                ...
                $lastName,
                $email,
                ($requestData['phone'] === '' ? null : ($requestData['phone'] ?? null)),
                ($requestData['date_of_birth'] === '' ? null : ($requestData['date_of_birth'] ?? null)),
                ($requestData['gender'] === '' ? null : ($requestData['gender'] ?? null)),
                ($requestData['nationality'] === '' ? null : ($requestData['nationality'] ?? null)),
                ($requestData['bank_account_no'] === '' ? null : ($requestData['bank_account_no'] ?? null)),
                ($requestData['bank_name'] === '' ? null : ($requestData['bank_name'] ?? null)),
                $requestData['employment_type'] ?? 'full_time',
                ($requestData['job_description'] === '' ? null : ($requestData['job_description'] ?? null)),
    ...
    public function listEmployees()
    {
        try {
            ...
            // Security Audit Fix: Standardize JSON decoding and apply data masking for non-admins
            $isAdmin = $this->hasAnyRole(['ADMIN', 'SUPERADMIN', 'SUPER_ADMIN', 'HRMANAGER', 'HR_MANAGER']);
            $myEmployeeId = $_SESSION['scope_employee_id'] ?? null;
            ...
            foreach ($employees as &$emp) {
                ...
                if (!$isAdmin && $emp['id'] != $myEmployeeId) {
                    // Mask sensitive PII in list view for non-privileged users
                    $emp['phone'] = '***';
                    $emp['date_of_birth'] = '***';
                    $emp['nationality'] = '***';
                    $emp['custom_data'] = [];
    ...
    public function updateSalary($requestData)
    {
        ...
        if (!$this->hasAnyRole(['ADMIN', 'SUPERADMIN', 'SUPER_ADMIN', 'HRMANAGER', 'HR_MANAGER'])) {
            return $this->jsonResponse(null, 403, "Security Violation: Only HR Managers or Admins can modify salary structures.");
        }
    ...
    public function getDashboardStats()
    {
        try {
            ...
            $queryCountry = "
                ...
                " . ($isGlobalAdmin ? "" : "WHERE e.id IN (SELECT ec2.employee_id FROM employee_companies ec2 WHERE ec2.is_active = 1 $companyFilter)") . "
