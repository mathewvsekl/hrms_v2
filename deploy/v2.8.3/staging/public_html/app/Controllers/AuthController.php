...
class AuthController extends Controller
{
    ...
    public function login()
    {
        ...
        try {
            ...
            $stmt = $db->prepare("
            SELECT u.id, u.password_hash, u.is_active, u.employee_id, ec.company_id, co.country_id, e.first_name, e.last_name, dg.title as designation 
            FROM users u 
        ...
        ");
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                // Noah Audit Fix: Fetch all associated companies for multi-office visibility
                if ($user['employee_id']) {
    ...
    public function requestOTP()
    {
        ...
        try {
            ...
            if (ENVIRONMENT === 'development') {
                $responseData['dev_otp'] = $otp; // Bypass helper for local dev
            }

            $this->jsonResponse($responseData, 200, "OTP requested successfully.");

        } catch (\Throwable $e) {
            $this->jsonResponse(null, 500, "SYSTEM ERROR: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
        }
    }
    ...
    public function verifyOTP()
    {
        ...
        try {
            ...
            // 1. Fetch User Record
            $stmt = $db->prepare("
                SELECT u.id, u.is_active, u.employee_id, ec.company_id, co.country_id, e.first_name, e.last_name, dg.title as designation 
                FROM users u 
                LEFT JOIN employees e ON u.employee_id = e.id
                LEFT JOIN employee_companies ec ON e.id = ec.employee_id AND ec.is_primary = 1 AND ec.is_active = 1
                LEFT JOIN companies co ON ec.company_id = co.id
