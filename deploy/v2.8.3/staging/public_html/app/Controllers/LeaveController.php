...
class LeaveController extends Controller
{
    ...
    public function submitRequest($requestData)
    {
        ...
        // Handle File Upload if present securely using UploadHelper
        if (isset($_FILES['attachment'])) {
            ...
            if ($uploadResult['success']) {
                ...
                // Only error out if a file was actually sent but failed validation
        ...
        try {
            ...
            foreach ($segments as $seg) {
                ...
                // 4. Validate Dates (New: No past dates for employees)
                $today = date('Y-m-d');
                if ($start < $today) {
                    return $this->jsonResponse(null, 400, "Leave cannot be requested for a past date (" . $start . ").");
                }

                // 5. Check Balance (Updated to include Pending)
                $stmtBal = $db->prepare("
                ...
                if ($remaining === false) {
                    // Fetch default allocation if no balance record exists yet
                ...
                if ($totalDays > $remaining) {
                    $db->rollBack();
                    return $this->jsonResponse(null, 400, "Insufficient balance for {$leaveType['name']}. Available: $remaining days (including pending requests).");
                }
