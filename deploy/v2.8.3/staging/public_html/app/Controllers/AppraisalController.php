...
class AppraisalController extends Controller
{
    ...
    public function getAppraisal($id)
    {
        ...
        try {
            ...
            // Fetch related data with template sections joined to avoid N+1 issues
            ...
            if ($role === 'EMPLOYEE') {
                foreach ($ratings as &$r) {
                    // Sections D_MANAGER and E_HR are restricted from employee view
    ...
    public function listAppraisals($params)
    {
        ...
        try {
            ...
            if ($role === 'EMPLOYEE' || ($role !== 'SUPERADMIN' && $role !== 'SUPER_ADMIN' && $role !== 'ADMIN' && !str_contains($role, 'HR'))) {
                // Standard Employee or basic Manager: only see what they manage or own
                $whereClause .= " AND (ea.manager_id = :mid OR ea.employee_id = :eid)";
                $queryParams['mid'] = $user['employee_id'];
                $queryParams['eid'] = $user['employee_id'];
            } else if ($role === 'ADMIN' || str_contains($role, 'HR')) {
                // HR/Admin: Apply Geographic Scoping
            ...
            // Hierarchical Isolation: Non-SuperAdmins cannot see SuperAdmin appraisals
            ...
            $query = "
                SELECT ea.*, e.first_name, e.last_name, ac.name as cycle_name
            ...
            return $this->jsonResponse($appraisals, 200, 'Appraisals retrieved successfully');
        } catch (\PDOException $e) {
            return $this->jsonResponse(['status' => 'error', 'message' => 'Database error: ' . $e->getMessage()], 500);
        }
    }
