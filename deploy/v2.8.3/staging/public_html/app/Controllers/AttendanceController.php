...
class AttendanceController extends Controller
{
    ...
    {
        return $this->jsonResponse($this->fetchLogs($requestData));
    }
        ...
        try {
            ...
            if (!$isGlobalAdmin) {
                ...
                if ($isMultiOffice && !empty($associatedCompanyIds)) {
                    ...
                    $geographicFilter = " AND cn.id = :session_country_id";
            ...
            if ($companyId) {
                $this->verifyDataScope($companyId);
                $geographicFilter .= " AND c.id = :company_id";
            ...
            $query = "
                ...
                " . ($isGlobalAdmin ? "" : "AND e.id IN (SELECT ec.employee_id FROM employee_companies ec WHERE ec.is_active = 1 $geographicFilter)") . "
            ";
            ...
            foreach ($logs as &$log) {
                $tz = \App\Helpers\DateHelper::getEmployeeTimezone($log['employee_id']);
    ...
    public function saveManualEntry($requestData)
    {
        ...
        $timezone = \App\Helpers\DateHelper::getEmployeeTimezone($employeeId);
        ...
        if ($checkIn) $checkIn = \App\Helpers\DateHelper::toUtc($checkIn, $timezone);
        if ($checkOut) $checkOut = \App\Helpers\DateHelper::toUtc($checkOut, $timezone);
    ...
    public function saveBulkEntry($requestData)
    {
        ...
        // Security Audit Fix: Explicit Scope Verification
        // Since this is a bulk entry, we should ideally verify the target company if provided, 
        // or ensure the user has permissions for the employees they are targeting.
        // For simplicity and safety, we enforce that non-admins can only bulk log for their own associated companies.
        if (!$this->isGlobalAdmin()) {
            $myCompanyIds = $_SESSION['associated_company_ids'] ?? [];
            if (empty($myCompanyIds)) {
                return $this->jsonResponse(null, 403, "Security Violation: You do not have permission to perform bulk attendance logging.");
            }
        }
        ...
        try {
            ...
            foreach ($employeeIds as $employeeId) {
                $timezone = \App\Helpers\DateHelper::getEmployeeTimezone($employeeId);
                $checkInUtc = $checkIn ? \App\Helpers\DateHelper::toUtc($checkIn, $timezone) : null;
                $checkOutUtc = $checkOut ? \App\Helpers\DateHelper::toUtc($checkOut, $timezone) : null;
    ...
    public function submitLogs($requestData)
    {
        ...
        try {
            ...
            // Security Audit Fix: Ownership/Scope check for submission
            // We only allow submission of logs for employees within the user's scope
            $scopeFilter = "";
            $scopeParams = [$userId];
            if (!$this->isGlobalAdmin()) {
                $myCompanyIds = $_SESSION['associated_company_ids'] ?? [];
            ...
            for ($d = 1; $d <= $daysInMonth; $d++) {
                ...
                if (isset($logsByDate[$dateStr])) {
                    ...
                    if (isset($leavePolicies[$st])) {
                        ...
                        // Noah Logic Auditor: If a custom status is used that isn't in policies, 
                        // check if it's a leave type code or name.
                        // By default, if it's a leave status, demote it unless explicit calendar policy exists.
    ...
    public function getMonthlyReport($requestData, $internalCall = false)
    {
        try {
            ...
            $systemCodes = [
                'weekend' => ['key' => 'WE', 'label' => 'Weekend', 'color_hex' => '#9ca3af', 'color' => 'custom'],
                'public_holiday' => ['key' => 'PU', 'label' => 'Public Holiday', 'color_hex' => '#3b82f6', 'color' => 'custom'],
                'holiday' => ['key' => 'HO', 'label' => 'Holiday', 'color_hex' => '#2563eb', 'color' => 'custom'],
                'present' => ['key' => 'PR', 'label' => 'Present', 'color_hex' => '#10b981', 'color' => 'custom'],
                'absent' => ['key' => 'AB', 'label' => 'Absent', 'color_hex' => '#ef4444', 'color' => 'custom'],
            ];

            // Fetch Office Attendance Status
            $officeStatusQuery = "SELECT status_key, status_label, color_code, display_code FROM office_attendance_status_definitions";
            $officeStatusParams = [];
            if ($companyId) {
            ...
            // Fetch Leave Types
            $leaveTypeQuery = "SELECT code, name, color_code, display_code FROM leave_types";
            ...
            $empQuery .= " GROUP BY e.id, e.employee_code, e.first_name, e.last_name, c.id, c.country_id ORDER BY e.first_name ASC";
