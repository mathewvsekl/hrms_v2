...
class AppraisalController extends Controller
{
        ...
        try {
            ...
            if (isset($data['ratings']) && is_array($data['ratings'])) {
                ...
                if ($user['role'] === 'Employee') {
                    ...
                    if ($kpiCount < 5) {
                        $this->jsonResponse(['status' => 'error', 'message' => 'Minimum 5 KPIs are required.'], 400);
                        return;
                    }
                }
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => 'Draft saved successfully']);
    ...
    public function initiateCycle($data)
    {
        ...
        if (!str_contains($role, 'HR') && !str_contains($role, 'ADMIN')) {
            $this->jsonResponse(['status' => 'error', 'message' => 'Unauthorized. Must be HR or Admin.'], 403);
            return;
        }
        ...
        try {
            ...
            foreach ($employees as $emp) {
                ...
                // Create Matrix Approvals (Level 1: Direct Manager, Level 2: Manager's Manager)
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => "Cycle initiated with " . count($employees) . " appraisals created."]);
        } catch (\PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $this->jsonResponse(['status' => 'error', 'message' => 'Database error: ' . $e->getMessage()], 500);
        }
    ...
    public function withdrawCycle($id)
    {
        ...
        try {
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => 'Cycle and pending appraisals withdrawn successfully.']);
        } catch (\PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $this->jsonResponse(['status' => 'error', 'message' => 'Database error: ' . $e->getMessage()], 500);
        }
    ...
    public function approveAppraisal($id, $data)
    {
        ...
        try {
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => 'Appraisal approved.']);
        } catch (\PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $this->jsonResponse(['status' => 'error', 'message' => 'Database error: ' . $e->getMessage()], 500);
        }
    ...
    public function returnAppraisal($id, $data)
    {
        ...
        try {
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => 'Appraisal returned to employee for review.']);
        } catch (\PDOException $e) {
            if ($this->db->inTransaction()) $this->db->rollBack();
            $this->jsonResponse(['status' => 'error', 'message' => 'Database error: ' . $e->getMessage()], 500);
        }
    ...
    public function finalize($id, $data)
    {
        ...
        try {
            ...
            $this->db->commit();
            $this->jsonResponse(['message' => 'Status updated to finalized with Payroll sync triggered.']);
    ...
    private function updateStatus($id, $status)
    {
        try {
            ...
            if ($stmt->execute([$status, $id])) {
                $this->jsonResponse(['message' => 'Status updated to ' . $status]);
            } else {
