...
class LeaveController extends Controller
{
    ...
    public function submitRequest($requestData)
    {
        ...
        try {
            ...
            foreach ($segments as $seg) {
                ...
                // 6. Insert
                $stmtIns = $db->prepare("
                    INSERT INTO leave_requests (employee_id, leave_type_id, start_date, end_date, total_days, status, request_group_id)
                    VALUES (:eid, :ltid, :start, :end, :days, 'pending', :gid)
                ");
                $stmtIns->execute([
                    'eid' => $employeeId,
                    'ltid' => $ltid,
    ...
    private function calculateLeaveDays($companyId, $leaveTypeId, $startDate, $endDate)
    {
        ...
        if (!$weekends) $weekends = ['Saturday', 'Sunday'];
    ...
    private function recalculateBalances($companyId, $employeeId = null)
    {
        ...
        $stmt = $db->prepare("
            SELECT clp.leave_type_id, clp.default_days_per_year, clp.is_calendar_days, clp.weekend_definition, lt.name as leave_type_name 
        ...
        foreach ($employees as $eid) {
            foreach ($policies as $policy) {
                ...
                $weekends = json_decode($policy['weekend_definition'] ?? '["Saturday", "Sunday"]', true) ?: ['Saturday', 'Sunday'];
    ...
    public function requestCancellation($requestData)
    {
        ...
        try {
            $db = \Database::getInstance()->getConnection();
            $stmt = $db->prepare("UPDATE leave_requests SET status = 'cancel_requested' WHERE id = ? AND status = 'approved'");
            $stmt->execute([$id]);
    ...
    public function adminCancel($requestData)
    {
        ...
        try {
            ...
            $attendance = new \App\Controllers\AttendanceController();
            $attendance->removeLeaveAttendanceRange($request['employee_id'], $request['start_date'], $request['end_date']);
