...
class LeaveController extends Controller
{
    ...
    public function savePolicy($data)
    {
        try {
            ...
            $syncStmt = $db->prepare("
                ...
                SELECT ec.employee_id, :ltid, :year, :days, 0
                FROM employee_companies ec
                WHERE ec.company_id = :cid AND ec.is_primary = 1 AND ec.is_active = 1
                ON DUPLICATE KEY UPDATE allocated_days = :days2
            ");
            $syncStmt->execute([
                'ltid' => $data['leave_type_id'],
                'year' => $year,
                'days' => $data['default_days_per_year'],
                'cid' => $data['company_id'],
                'days2' => $data['default_days_per_year']
            ]);
    ...
    public function deleteLeaveType($id)
    {
        try {
            $db = \Database::getInstance()->getConnection();
            // Optional: check if there are associated requests or balances
