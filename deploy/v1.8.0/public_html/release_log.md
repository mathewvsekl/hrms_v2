...
- **OTP Reliability**: Fixed an issue where trailing spaces in email inputs prevented OTP generation.
- **Enhanced Logging**: Integrated manual diagnostic logging into `MailHelper` and `AuthController` to allow real-time troubleshooting of authentication failures via server logs.
