    ...
    public function getEmployeePayslips($employeeId = null)
    {
        try {
            ...
            if (!$isGlobalAdmin && ($employeeId === null || $employeeId != $myEmployeeId)) {
                ...
                $rbacClause = "(p.company_id IN ($placeholders) OR (p.company_id IS NULL AND EXISTS (SELECT 1 FROM employee_companies ec WHERE ec.employee_id = p.employee_id AND ec.is_primary = 1 AND ec.company_id IN ($placeholders))))";
